...
def call_ai_once(provider: str, key: str, model: str, user_text: str, catalog: List[dict]) -> dict:
    ...
    with urllib.request.urlopen(req, timeout=int(os.getenv("PRICEBOT_TEXT_AI_TIMEOUT", "7"))) as response:
...
def _call_openrouter_vision_json(media_bytes: bytes, mime_type: str, provider: str = "openrouter") -> dict:
    ...
    for index, key in enumerate(keys, 1):
        ...
        try:
            r = requests.post(endpoint, headers=headers, json=payload, timeout=int(os.getenv("PRICEBOT_VISION_TIMEOUT", "22")))
...
def _call_direct_gemini_vision_json(media_bytes: bytes, mime_type: str) -> dict:
    ...
    for index, key in enumerate(keys, 1):
        ...
        try:
            with urllib.request.urlopen(req, timeout=int(os.getenv("PRICEBOT_VISION_TIMEOUT", "22"))) as response:
