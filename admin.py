...
async def dashboard(key: str = Depends(verify_admin)):
    ...
    html_content += f"""
        ...
        <div style="display: flex; gap: 20px; justify-content: center; margin-top: 20px;">
            <div style="background: #ecf0f1; padding: 20px; border-radius: 8px; text-align: center; flex: 1;">
                <h2>📦 إجمالي المنتجات</h2>
                <p style="font-size: 24px;"><b>{len(products)}</b> منتج</p>
            </div>
            <div style="background: #fcf3cf; padding: 20px; border-radius: 8px; text-align: center; flex: 1;">
...
async def manage_products(key: str = Depends(verify_admin)):
    ...
    html_content += f"""
        ...
        <form action="/admin/upload?key={key}" method="post" enctype="multipart/form-data" style="background: #e8f8f5; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
            ...
            <p style="font-size: 12px; color: gray;">* إذا لم تقم بتحديد المربع، سيتم تحديث الأدوية الموجودة وإضافة الجديدة فقط (Upsert الآمن).</p>
    ...
    for p in products[:50]: 
        ...
        """
    html_content += "</table><p style='text-align:center; color:gray; font-size:12px;'>يتم عرض أول 50 منتج فقط لتسريع الصفحة.</p></div></body></html>"
    return html_content
...
async def upload_file(
    ...
    try:
        # دعم الإكسيل والـ CSV (النقطة 24)
        ...
        with database.get_db_connection() as conn:
            ...
            for row in parsed_data:
                ...
                available = str(row.get("available", "متوفر"))
...
async def delete_product(product_id: int, key: str = Depends(verify_admin)):
    # (النقطة 25) نسخة احتياطية قبل الحذف
    database.backup_database()
    with database.get_db_connection() as conn:
...
async def manage_orders(key: str = Depends(verify_admin)):
    ...
    html_content += """
        ...
        <table>
            <tr><th>رقم الطلب</th><th>رقم هاتف الزبون</th><th>المنتج المطلوب</th><th>التاريخ</th><th>الحالة</th><th>إجراء</th></tr>
    """
    for o in orders:
        status = o.get('status', 'pending')
        badge_class = "bg-pending" if status == "pending" else "bg-completed" if status == "completed" else "bg-canceled"
        status_text = "قيد الانتظار" if status == "pending" else "مكتمل" if status == "completed" else "ملغي"
        ...
        html_content += f"""
            <tr>
                <td>#{o['id']}</td>
                <td dir="ltr">{safe_phone}</td>
                <td>{safe_product}</td>
                <td dir="ltr">{o['created_at']}</td>
                ...
                <td>
                    <form action="/admin/orders/update/{o['id']}?key={key}" method="post" style="display:flex; gap: 5px;">
                        <select name="new_status" style="padding: 5px;">
                            ...
                            <option value="canceled" {'selected' if status=='canceled' else ''}>ملغى</option>
                        </select>
                        <button type="submit" class="btn btn-success" style="padding: 5px 10px; margin: 0;">تحديث</button>
                    </form>
                </td>
            </tr>
        """
