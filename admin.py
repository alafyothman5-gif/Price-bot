...
<a href="/admin">الرئيسية</a><a href="/admin/analytics">الإحصائيات</a><a href="/admin/customer-link">رابط الزبائن/QR</a><a href="/admin/products">المنتجات</a><a href="/admin/orders">الطلبات</a><a href="/admin/failed-queries">الاستعلامات الفاشلة</a><a href="/admin/misses">Misses</a><a href="/admin/alias-learning">Alias Learning</a><a href="/admin/catalog-quality">Catalog Quality</a><a href="/admin/synonyms">Synonyms</a><a href="/admin/logout">خروج</a>
</div></div>
"""
...
async def customer_link_page(request: Request, phone: str = "", text: str = "السلام عليكم", _: bool = Depends(require_admin)):
    ...
    if number:
        ...
        content += '<h1 style="margin:12px 0 4px">صيدلية بدر البشرية</h1><p style="font-size:20px">امسح الكود للاستفسار عن السعر والتوفر عبر واتساب</p><button class="btn" onclick="window.print()">طباعة</button></div>'
    else:
...
async def edit_product(product_id: int, request: Request, _: bool = Depends(require_admin)):
    ...
    fields=["product_id","name","price","brand","company","category","product_family","form","size","aliases","image_ocr_keywords","ocr_keywords","active_ingredient","use_case","skin_type","strength","pack","available","substitution_group_id","review_status","review_notes","code","barcode","sku","item_code","product_code"]
...
HEADER_ALIASES={
    ...
    "code":{"code","كود"},"barcode":{"barcode","باركود"},"sku":{"sku"},"item_code":{"item_code"},"product_code":{"product_code"}
}
...
def upsert_products(products: List[dict], replace_all: bool)->int:
    ...
    _database.init_db()
    import_columns=["product_id","name","price","company","brand","category","product_family","form","size","aliases","image_ocr_keywords","ocr_keywords","active_ingredient","use_case","skin_type","available","strength","pack","substitution_group_id","review_status","review_notes","code","barcode","sku","item_code","product_code"]
...
async def query_misses(request: Request, _: bool = Depends(require_admin)):
    ...
    for row in rows:
        ...
        content += f'<tr><td>{raw}</td><td>{clean}</td><td>{decision}</td><td>{freq}</td><td dir="ltr">{last}</td><td><form method="post" action="/admin/addsynonym"><input name="source" value="{raw}"><input name="target" placeholder="target مثل cerave"><button class="btn">حفظ</button></form></td></tr>'
    content += '</table></div>'
    return HTMLResponse(content + html_footer())
...
async def catalog_quality(request: Request, _: bool = Depends(require_admin)):
    ...
    content += '<div class="card"><h2>تقرير جودة الكتالوج</h2><p class="muted">هذا التقرير لا يغير البيانات. يستخدم لكشف الأعمدة الناقصة التي تؤثر على المطابقة: brand/form/strength/aliases/type.</p><a class="btn" href="/admin/catalog-quality.csv">تحميل CSV</a></div>'
    content += '<div class="grid">'
    content += metric_card("عدد المنتجات", str(len(rows)), "كل المنتجات المقروءة من SQLite", "blue")
    content += metric_card("Missing brand", str(issue_counts.get("missing_brand", 0)), "منتجات بلا شركة واضحة", "orange")
    content += metric_card("Missing form/type", str(issue_counts.get("missing_form_or_type", 0)), "دواء/كوزمتك بلا شكل/نوع", "red")
    content += metric_card("Weak aliases/OCR", str(issue_counts.get("weak_aliases_or_ocr_keywords", 0)), "تحتاج aliases أو OCR keywords", "purple")
    content += '</div>'
    content += '<div class="card"><h2>أول 250 ملاحظة</h2><table><tr><th>ID</th><th>الاسم</th><th>Brand</th><th>Form</th><th>Strength</th><th>Category</th><th>Issues</th></tr>'
    shown = 0
    for row in rows:
        ...
        if shown > 250:
            break
        content += f'<tr><td>{h(row.get("id"))}</td><td>{h(row.get("name"))}</td><td>{h(row.get("brand"))}</td><td>{h(row.get("form"))}</td><td>{h(row.get("strength"))}</td><td>{h(row.get("category"))}</td><td>{h(row.get("issues"))}</td></tr>'
    content += '</table></div>'
    return HTMLResponse(content + html_footer())
...
async def catalog_quality_csv(request: Request, _: bool = Depends(require_admin)):
    ...
    return Response(content=output.getvalue(), media_type="text/csv; charset=utf-8", headers={"Content-Disposition": "attachment; filename=catalog_quality_v4.csv"})
...
async def alias_learning(request: Request, _: bool = Depends(require_admin)):
    ...
    for row in misses:
        ...
        content += f'<tr><td>{h(raw)}</td><td>{h(clean)}</td><td>{h(row.get("decision"))}</td><td>{h(row.get("freq"))}</td><td><form method="post" action="/admin/alias-learning/approve"><input type="hidden" name="source_query" value="{h(raw)}"><input type="hidden" name="clean_query" value="{h(clean)}"><input name="product_id" placeholder="Product ID" required><button class="btn btn-success">اعتماد</button></form></td></tr>'
    content += '</table></div>'
...
async def alias_learning_approve(request: Request, source_query: str = Form(...), clean_query: str = Form(""), product_id: int = Form(...), _: bool = Depends(require_admin)):
    database.backup_database()
...
async def synonyms_page(request: Request, _: bool = Depends(require_admin)):
    ...
    content += '<div class="card"><h2>إضافة مرادف</h2><form method="post" action="/admin/addsynonym"><label>Source</label><input name="source" required placeholder="مثال: سيرا في"><label>Target</label><input name="target" required placeholder="مثال: cerave"><button class="btn btn-success" type="submit">إضافة بدون restart</button></form></div>'
...
async def add_synonym(request: Request, source: str = Form(...), target: str = Form(...), _: bool = Depends(require_admin)):
