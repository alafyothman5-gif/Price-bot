...
def ai_provider_order() -> List[str]:
    raw = get_config("AI_PROVIDER_ORDER", "openrouter")
...
def find_product(text: str) -> Optional[dict]:
    resolved = resolve_product_query(text)
    if resolved:
        return resolved
...
def call_ai_once(provider: str, key: str, model: str, user_text: str, catalog: List[dict]) -> dict:
    ...
    with urllib.request.urlopen(req, timeout=int(os.getenv("AI_TEXT_TIMEOUT_SECONDS", os.getenv("PRICEBOT_TEXT_AI_TIMEOUT", "8")))) as response:
...
def query_product_matches(text: str, limit: int = 8) -> List[dict]:
    ...
    if not qn:
        return []
    resolved = resolve_product_query(q)
    if resolved:
        return [resolved]
    if is_weak_only_query(q):
        return []
    # EXACT_MATCH_HOTFIX_V1: exact catalog hit wins before fuzzy ranking.
    exact = exact_safe_product_match(q)
    if exact:
        return [exact]
...
def build_reply(text: str, from_number: str = "") -> str:
    ...
    # EXACT_MATCH_HOTFIX_V1:
    # If the customer's text contains an exact catalog product name/alias, answer it immediately.
    # Do this before fuzzy/safe-unavailable logic so an existing product is never reported
    # as unavailable and then shown as its own alternative.
    exact_item = resolve_product_query(raw_text)
    if exact_item:
        if from_number:
            ...
            PENDING_OPTIONS.pop(from_number, None)
            PENDING_ALT_REQUESTS.pop(from_number, None)
        log_product_inquiry(raw_text, from_number, exact_item.get("name", ""), True, "exact_catalog_match")
    ...
    if is_weak_only_query(raw_text):
        log_product_inquiry(raw_text, from_number, "", False, "weak_query_rejected")
...
def image_ai_enabled() -> bool:
    """Vision AI is enabled when AI is enabled and any configured vision-capable provider has a key.
    This supports OpenRouter keys, not only direct Gemini keys.
    """
    if not config_bool("AI_ENABLED", False):
        return False
    for provider in ai_provider_order():
        if provider in {"openrouter", "gemini", "openai", "custom"} and ai_keys_for_provider(provider):
            return True
    return False


def _vision_prompt() -> str:
    # PRICEBOT_FAST_VISION_V1:
    # Keep the prompt short. Do NOT send the product catalog to the AI.
    # AI extracts visible identity only; local code/database makes the final decision.
    return (
        "أرجع JSON فقط بدون شرح. اقرأ صورة منتج صيدلية أو كوزمتك. "
        "لا تخترع. اقرأ النص الظاهر فقط. إذا النص غير واضح اجعل clarity=bad وconfidence منخفض. "
        "إذا كانت روشتة/وصفة/أدوية كثيرة/صورة غير واضحة اجعل image_type=prescription_or_unclear. "
        "إذا كانت علبة منتج واضحة اجعل image_type=product_packaging. "
        "استخرج: brand, product_names, visible_text, product_type, size. "
        "فرّق بدقة: cleanser/غسول/منظف، moisturizer/مرطب، lotion/لوشن، cream/كريم، serum/سيروم، sunscreen/spf/واقي شمس، tablets/capsules/syrup/injection. "
        "إذا ظهر Cleanser أو Crème Lavante أو غسول أو Foaming أو Wash فـ product_type=cleanser. "
        "إذا ظهر Lotion فـ product_type=lotion. إذا ظهر Moisturizing Cream فـ product_type=moisturizer أو cream. "
        "الشكل المطلوب فقط: "
        "{\"image_type\":\"product_packaging|prescription_or_unclear|unknown\",\"product_names\":[],\"visible_text\":\"\",\"brand\":\"\",\"product_type\":\"\",\"size\":\"\",\"requires_admin_review\":false,\"confidence\":0.0,\"clarity\":\"good|medium|bad\"}"
    )


def resize_image_for_vision(media_bytes: bytes, mime_type: str) -> Tuple[bytes, str]:
    """Reduce image size before sending to vision model for speed and lower cost."""
    if not media_bytes or Image is None:
        return media_bytes, mime_type or "image/jpeg"
    try:
        max_px = int(os.getenv("PRICEBOT_VISION_MAX_PX", "1024"))
        quality = int(os.getenv("PRICEBOT_VISION_JPEG_QUALITY", "78"))
        with Image.open(io.BytesIO(media_bytes)) as img:
            ...
            print(f"VISION IMAGE RESIZED: old={len(media_bytes)} new={len(new_bytes)} size={img.size}", flush=True)
            return new_bytes, "image/jpeg"
    except Exception as exc:
        print("VISION IMAGE RESIZE SKIP:", exc, flush=True)
        return media_bytes, mime_type or "image/jpeg"


def _call_openrouter_vision_json(media_bytes: bytes, mime_type: str, provider: str = "openrouter") -> dict:
    keys = ai_keys_for_provider(provider)
    if not keys or not media_bytes:
        return {}
    model = ai_model_for(provider) or "google/gemini-2.5-flash-lite"
    endpoint = ai_endpoint_for(provider).strip() or "https://openrouter.ai/api/v1/chat/completions"
    # Accept either base URL or full chat/completions endpoint.
    if endpoint.rstrip("/").endswith("/v1"):
        endpoint = endpoint.rstrip("/") + "/chat/completions"
    prompt = _vision_prompt()
    media_bytes, mime_type = resize_image_for_vision(media_bytes, mime_type)
    ...
    for index, key in enumerate(keys, 1):
        headers = {
            ...
            "Content-Type": "application/json",
            "HTTP-Referer": get_config("AI_OPENROUTER_REFERER", public_base_url()),
            "X-Title": get_config("AI_OPENROUTER_TITLE", "PriceBot"),
        }
        payload = {
            "model": model,
            "messages": [{
        ...
        try:
            r = requests.post(endpoint, headers=headers, json=payload, timeout=int(os.getenv("AI_IMAGE_TIMEOUT_SECONDS", os.getenv("PRICEBOT_VISION_TIMEOUT", "20"))))
            if r.status_code != 200:
                print(f"VISION AI HTTP ERROR: provider={provider} model={model} status={r.status_code} body={r.text[:700]}", flush=True)
                continue
            raw = r.json()
            content = raw.get("choices", [{}])[0].get("message", {}).get("content", "") or ""
            data = json_from_model_text(content)
            if data:
                print(f"VISION AI OK: provider={provider} model={model} key_index={index} parsed={data}", flush=True)
                memory_record_ai_usage("image", provider)
                return data
            print(f"VISION AI PARSE EMPTY: provider={provider} model={model} content={content[:500]}", flush=True)
        except Exception as exc:
            if "timed out" in str(exc).lower() or "timeout" in str(exc).lower():
                print(f"VISION AI TIMEOUT: provider={provider} model={model} error={exc}", flush=True)
                return {"image_type": "timeout", "confidence": 0.0, "clarity": "bad"}
            print(f"VISION AI EXCEPTION: provider={provider} model={model} error={exc}", flush=True)
    return {}


def _call_direct_gemini_vision_json(media_bytes: bytes, mime_type: str) -> dict:
    ...
    for index, key in enumerate(keys, 1):
        ...
        try:
            with urllib.request.urlopen(req, timeout=int(os.getenv("AI_IMAGE_TIMEOUT_SECONDS", os.getenv("PRICEBOT_VISION_TIMEOUT", "20")))) as response:
                ...
                data = json_from_model_text(content)
                if data:
                    print(f"VISION AI OK: provider=gemini model={model} key_index={index} parsed={data}", flush=True)
                    memory_record_ai_usage("image", "gemini")
                    return data
        except urllib.error.HTTPError as exc:
            print("VISION AI HTTP ERROR: provider=gemini", exc.code, exc.read().decode("utf-8", errors="ignore")[:700], flush=True)
        except Exception as exc:
            if "timed out" in str(exc).lower() or "timeout" in str(exc).lower():
                print("VISION AI TIMEOUT: provider=gemini", str(exc), flush=True)
                return {"image_type": "timeout", "confidence": 0.0, "clarity": "bad"}
            print("VISION AI EXCEPTION: provider=gemini", str(exc), flush=True)
    return {}


def call_gemini_vision_json(media_bytes: bytes, mime_type: str) -> dict:
    """Compatibility name. Actually calls the configured provider order, including OpenRouter vision.
    The final product decision is still made by local safe matching.
    """
    if not image_ai_enabled():
        print("VISION AI DISABLED: no enabled provider key", flush=True)
        return {}
    for provider in ai_provider_order():
        if provider == "gemini" and ai_keys_for_provider("gemini"):
            data = _call_direct_gemini_vision_json(media_bytes, mime_type)
        elif provider in {"openrouter", "openai", "custom"} and ai_keys_for_provider(provider):
            data = _call_openrouter_vision_json(media_bytes, mime_type, provider)
        else:
            data = {}
        if data:
            return data
...
REQUEST_WORDS = {
    "عندكم", "متوفر", "متوفرة", "السعر", "سعر", "بكم", "كم", "ابي", "أبي", "نبي", "موجود", "موجودة",
    "available", "price", "do you have", "have", "how much", "please", "need", "want",
}

IMPORTANT_PRODUCT_WORDS = {
    "baume", "serum", "cleanser", "lotion", "cream", "foaming", "hydrating", "sa", "daily",
    "moisturising", "moisturizing", "gel", "oil", "sunscreen", "shampoo", "toner", "moisturizer",
}

WEAK_MATCH_WORDS = {
    "cerave", "vichy", "bioderma", "eucerin", "acm", "svr", "uriage", "avene",
    "lotion", "cream", "serum", "gel", "skin", "daily", "hydrating", "moisturising", "moisturizing",
}

BRAND_ONLY_ALIASES = {normalize(x) for x in [
    "cerave", "cera ve", "vichy", "bioderma", "eucerin", "acm", "svr", "uriage", "avene",
]}


def normalize_product_query(text: str, remove_request_words: bool = True) -> str:
    """Normalize product identity text without dropping meaningful product words."""
    q = normalize(text or "")
    if not q:
        return ""
    q = re.sub(r"\bcera\s+ve\b", "cerave", q)
    q = q.replace("moisturising", "moisturizing")
    q = q.replace("moisturiser", "moisturizer")
    q = q.replace("crème", "creme").replace("creme", "cream")
    if remove_request_words:
        for phrase in sorted((normalize_product_query(w, False) for w in REQUEST_WORDS), key=len, reverse=True):
            if not phrase:
                continue
...
def product_identity_tokens(text: str) -> List[str]:
    q = normalize_product_query(text, remove_request_words=False)
    return [t for t in q.split() if t]


def is_weak_only_query(text: str) -> bool:
    tokens = product_identity_tokens(text)
    if not tokens:
        return True
    weak = {normalize_product_query(w, False) for w in WEAK_MATCH_WORDS}
    return all(t in weak for t in tokens)


def is_brand_only_alias(term: str) -> bool:
    n = normalize_product_query(term, remove_request_words=False)
    return n in BRAND_ONLY_ALIASES


def product_name_alias_terms(item: dict) -> List[Tuple[str, str, str]]:
    terms: List[Tuple[str, str, str]] = []
    name = str(item.get("name", "") or "").strip()
    if name:
        terms.append(("name", name, normalize_product_query(name, remove_request_words=False)))
    for alias in split_aliases(str(item.get("aliases", "") or "")):
        alias = alias.strip()
        if not alias:
            continue
        norm_alias = normalize_product_query(alias, remove_request_words=False)
        if not norm_alias or is_brand_only_alias(alias) or is_weak_only_query(alias):
            continue
        terms.append(("alias", alias, norm_alias))
    clean: List[Tuple[str, str, str]] = []
    seen = set()
    for kind, raw, norm in terms:
        if norm and norm not in seen:
            clean.append((kind, raw, norm))
            seen.add(norm)
    return clean


def product_type_compatible(query: str, item: dict) -> bool:
    qtypes = detect_product_types(query)
    itypes = product_types(item)
    if has_type_conflict(qtypes, itypes):
        return False
    cosmetic_qtypes = qtypes & COSMETIC_TYPES
    cosmetic_itypes = itypes & COSMETIC_TYPES
    if cosmetic_qtypes and cosmetic_itypes and not (cosmetic_qtypes & cosmetic_itypes):
        return False
    return True


def meaningful_fuzzy_query(query: str) -> bool:
    tokens = product_identity_tokens(query)
    if len(tokens) < 2:
        return False
    if is_weak_only_query(query):
        return False
    important = {normalize_product_query(w, False) for w in IMPORTANT_PRODUCT_WORDS}
    weak = {normalize_product_query(w, False) for w in WEAK_MATCH_WORDS}
    return any(t not in weak or t in important for t in tokens)


def resolve_product_query(query: str) -> Optional[dict]:
    """Resolve a customer/image product query using strict local catalog matching only."""
    q = normalize_product_query(extract_product_query(query or "") or query or "")
    if not q:
        return None
    products = load_products()

    # 1) Exact normalized product name wins immediately.
    for item in products:
        name_norm = normalize_product_query(item.get("name", ""), remove_request_words=False)
        if q == name_norm:
            return item

    # 2) Exact alias wins, except brand-only/weak-only aliases.
    for item in products:
        if not product_type_compatible(q, item):
            continue
        for kind, raw, term_norm in product_name_alias_terms(item):
            if kind == "alias" and q == term_norm:
                return item

    # 3) Full phrase contained in a product name or safe alias.
    phrase_matches: List[Tuple[float, dict]] = []
    if not is_weak_only_query(q):
        for item in products:
            if not product_type_compatible(q, item):
                continue
            for kind, raw, term_norm in product_name_alias_terms(item):
                if len(q) >= 4 and q in term_norm:
                    score = len(q) / max(len(term_norm), 1)
                    phrase_matches.append((score, item))
                    break
    if phrase_matches:
        phrase_matches.sort(key=lambda x: x[0], reverse=True)
        return phrase_matches[0][1]

    # 4) Strict fuzzy only after exact/phrase matching fails.
    if not meaningful_fuzzy_query(q):
        return None
    fuzzy: List[Tuple[float, dict]] = []
    for item in products:
        if not product_type_compatible(q, item):
            continue
        if is_specific_product_query(q) and not required_terms_present(q, item):
            continue
        best = 0.0
        for kind, raw, term_norm in product_name_alias_terms(item):
            if kind == "alias" and (is_brand_only_alias(raw) or is_weak_only_query(raw)):
                continue
            best = max(best, SequenceMatcher(None, q, term_norm).ratio())
        if best >= 0.94:
            fuzzy.append((best, item))
    fuzzy.sort(key=lambda x: x[0], reverse=True)
    if not fuzzy:
        return None
    if len(fuzzy) > 1 and fuzzy[0][0] - fuzzy[1][0] < 0.04:
        return None
    return fuzzy[0][1]
...
def cosmetic_alternatives_for_query(text: str, limit: int = 6) -> List[dict]:
    ...
    for require_brand in [True, False]:
        for score, item in ranked_products(text):
            if not is_product_available(item):
                continue
            itypes = product_types(item)
...
def is_product_available(item: dict) -> bool:
    status = normalize(str(item.get("available", "متوفر") or "متوفر"))
    return status not in {"غير متوفر", "غير موجود", "نفذ", "ناقص", "لا", "no", "0"}


def build_cosmetic_alternatives_direct_reply(text: str, alts: List[dict], from_number: str = "") -> str:
    alts = [a for a in alts if is_product_available(a)]
    if from_number and alts:
        PENDING_OPTIONS[from_number] = {"items": alts[:8], "reason": "cosmetic_alternatives"}
    lines = [
        f"{pharmacy_name()} 🌿",
        "",
    ...
    if not alts:
        lines.append("لا توجد بدائل كوزمتك قريبة متوفرة حالياً.")
        return "\n".join(lines)

    best = alts[0]
    lines += [
        "",
        "بدائل كوزمتك متوفرة قريبة من نفس النوع:",
        f"⭐ أنسب بديل مقترح: {product_label(best)}",
    ]
    price = str(best.get("price", "")).strip()
    if price:
        lines.append(f"💰 السعر: {price}")
    lines.append("")
    for i, item in enumerate(alts[:5], 1):
        price = str(item.get("price", "")).strip()
        suffix = f" — {price}" if price else ""
        lines.append(f"{i}) {product_label(item)}{suffix}")
    lines += ["", "اكتب رقم البديل المطلوب للحجز أو الاستفسار."]
    return "\n".join(lines)


def build_unavailable_reply(text: str, from_number: str = "") -> str:
    # EXACT_MATCH_HOTFIX_V1 safety guard: never say unavailable if the exact product exists.
    exact = resolve_product_query(text)
    if exact:
        if from_number:
            LAST_PRODUCT[from_number] = exact
            PENDING_ALT_REQUESTS.pop(from_number, None)
        return build_product_reply(exact, text)
    alts = [a for a in cosmetic_alternatives_for_query(text, limit=8) if is_product_available(a)]
    # Do not include the exact requested product as an alternative if a future matcher ever adds it.
    nq = normalize(extract_product_query(text) or text)
    if nq:
        alts = [a for a in alts if normalize(a.get("name", "")) not in nq and nq not in normalize(a.get("name", ""))]
    if from_number:
        PENDING_ALT_REQUESTS.pop(from_number, None)
    if alts:
        return build_cosmetic_alternatives_direct_reply(text, alts, from_number)
    return "\n".join([
        f"{pharmacy_name()} 🌿",
        "",
...
def build_unclear_image_reply() -> str:
    return (
        f"{pharmacy_name()} ًںŒ؟\n\n"
        "الصورة غير واضحة بما يكفي للتعرف على المنتج. الرجاء إرسال صورة أوضح أو كتابة اسم المنتج."
    )


def build_image_timeout_reply() -> str:
    return (
        f"{pharmacy_name()} ًںŒ؟\n\n"
        "الصورة أخذت وقتاً أطول من المعتاد. الرجاء إرسال صورة أوضح أو كتابة اسم المنتج."
    )
...
def product_from_vision(data: dict) -> Optional[dict]:
    ...
    names: List[str] = []
    for key in ["product_name", "product_names", "visible_text", "brand", "matched_product_names"]:
        val = data.get(key)
    ...
    product_type = str(data.get("type", "") or data.get("product_type", "") or "").strip()
    ...
    for name in combined_names:
        ...
        if not n or n in seen:
            continue
        seen.add(n)
        item = resolve_product_query(name)
        if item:
            return item
    return None
...
def process_image_message(msg: dict, from_number: str) -> str:
    ...
    # Send an immediate message so the customer does not wait silently while Vision AI runs.
    try:
        send_whatsapp_message(from_number, f"{pharmacy_name()} 🌿\n\nجاري فحص الصورة، انتظر لحظات...")
    except Exception as exc:
        print("IMAGE ACK SEND SKIP:", exc, flush=True)
    ...
    if cached_image:
        # SAFE_VISION_OPENROUTER_FIX_V2:
        # Do NOT trust old image->product cache. A previous weak matcher may have cached a wrong product
        # such as cleanser image -> moisturizer product. Re-read product images with the current AI matcher.
        if cached_image.get("type") == "review":
            ...
            return build_image_under_review_reply()
        print(f"VISION CACHE IGNORED: stale product cache sha={media_sha[:12]}", flush=True)

    vision = call_gemini_vision_json(media_bytes, mime_type) if image_ai_enabled() else {}
    try:
        print(f"IMAGE PROCESS VISION DONE seconds={(datetime.now()-start_ts).total_seconds():.1f} vision={str(vision)[:500]}", flush=True)
    except Exception:
        pass
    ...
    has_product_identity = bool(
        vision.get("brand") or vision.get("product_name") or vision.get("product_names") or vision.get("visible_text")
    )

    if image_type == "timeout":
        memory_remember_image_review(media_sha, "vision_timeout")
        return build_image_timeout_reply()

    if not vision or requires_review or image_type in {"prescription_or_unclear", "unknown"} or (image_type == "" and not has_product_identity) or clarity == "bad" or confidence < 0.55:
        ...
        try:
            _pb_threading.Thread(target=notify_admin, args=(
    ...
    if image_type == "product_packaging" or has_product_identity:
        ...
        product_text = " ".join([product_names, visible_text, str(vision.get("brand", "") or ""), str(vision.get("type", "") or vision.get("product_type", "") or ""), str(vision.get("size", "") or "")]).strip()
        if item:
...
def admin_settings(key: str = "", msg: str = ""):
    ...
    <div class="box">
      ...
      <form method="post" action="/admin/settings/save?key={urllib.parse.quote(key)}">
        ...
        <div class="form-grid">
          <div class="field"><label>ترتيب المحاولة</label><input name="AI_PROVIDER_ORDER" value="{e(order_value)}" placeholder="openrouter"></div>
          <div class="field"><label>OpenRouter model</label><input name="AI_OPENROUTER_MODEL" value="{e(ai_model_for('openrouter'))}" placeholder="google/gemini-2.0-flash-001"></div>
...
async def admin_settings_save(request: Request, key: str = ""):
    ...
    values = {
        ...
        "AI_PROVIDER_ORDER": str(form.get("AI_PROVIDER_ORDER", ",".join(ai_provider_order()))).strip() or "openrouter",
        ...
        if last_from_number:
            try:
                send_whatsapp_message(
                    last_from_number,
                    f"{pharmacy_name()} ًںŒ؟\n\nحدث خطأ مؤقت أثناء معالجة الرسالة. الرجاء إعادة إرسال اسم المنتج أو صورة أوضح.",
                )
            except Exception as send_exc:
                print("WEBHOOK FALLBACK SEND ERROR:", str(send_exc), flush=True)
...
# =============================================================================
# PRICEBOT_FINAL_SAFE_MATCH_V2
# Final local product decision layer. Loaded before uvicorn.run so it works when
# the server is started with either `python app.py` or `uvicorn app:app`.
# =============================================================================

_PB_SEQUENCE_MATCHER = __import__("difflib").SequenceMatcher

_FINAL_REQUEST_PHRASES = {
    "متوفر", "متوفرة", "عندكم", "السعر", "سعر", "بكم", "كم", "هل", "نبي", "ابي", "أبي",
    "موجود", "موجودة", "لو سمحت", "من فضلك", "available", "price", "do you have",
    "how much", "have", "need", "want", "please",
}
...
_FINAL_WEAK_WORDS = {
    "cerave", "vichy", "bioderma", "eucerin", "acm", "svr", "uriage", "avene",
    "lotion", "cream", "serum", "gel", "skin", "daily", "hydrating", "moisturizing",
    "moisturising",
}

_FINAL_IMPORTANT_WORDS = {
    "baume", "serum", "cleanser", "lotion", "cream", "foaming", "hydrating", "sa",
    "daily", "moisturizing", "gel", "oil", "sunscreen", "shampoo", "toner", "moisturizer",
}
...
_FINAL_TYPE_SYNONYMS = {
    "cleanser": ["cleanser", "cleansing", "wash", "foaming", "gel moussant", "lavante", "غسول", "منظف"],
    "moisturizer": ["moisturizer", "moisturizing", "moisturising", "moisturiser", "مرطب"],
    "lotion": ["lotion", "لوشن"],
    "serum": ["serum", "سيروم"],
    "cream": ["cream", "creme", "crème", "كريم"],
    "sunscreen": ["sunscreen", "sun screen", "spf", "واقي شمس"],
    "toner": ["toner", "تونر"],
    "gel": ["gel"],
    "oil": ["oil"],
    "shampoo": ["shampoo", "شامبو"],
    "syrup": ["syrup", "syp", "شراب"],
    "tablets": ["tablet", "tablets", "tab", "tabs", "اقراص", "أقراص", "حبوب"],
    "capsules": ["capsule", "capsules", "cap", "caps", "كبسول", "كبسولات"],
    "injection": ["injection", "inj", "amp", "ampoule", "vial", "حقن", "امبول", "أمبول"],
}
...
def _final_norm(text: str, strip_request_words: bool = False) -> str:
    ...
    for src, dst in {
        ...
        s = s.replace(src, dst)
    s = re.sub(r"[\u064b-\u065f\u0670]", "", s)
    for src, dst in sorted(_FINAL_SYNONYMS.items(), key=lambda x: len(x[0]), reverse=True):
        s = re.sub(rf"(?<!\w){re.escape(src)}(?!\w)", dst, s)
    s = re.sub(r"[^\w\s\u0600-\u06FF]+", " ", s)
    s = re.sub(r"\bcera\s+ve\b", "cerave", s)
    if strip_request_words:
        for phrase in sorted(_FINAL_REQUEST_PHRASES, key=len, reverse=True):
            p = _final_norm(phrase, False)
            if p:
                s = re.sub(rf"(?<!\w){re.escape(p)}(?!\w)", " ", s)
    return re.sub(r"\s+", " ", s).strip()


def normalize_product_query(text: str, remove_request_words: bool = True) -> str:
    return _final_norm(text, strip_request_words=remove_request_words)


def _final_tokens(text: str) -> List[str]:
    return [t for t in _final_norm(text, False).split() if t]


def is_weak_only_query(text: str) -> bool:
    tokens = _final_tokens(text)
    return not tokens or all(t in _FINAL_WEAK_WORDS for t in tokens)


def _final_detect_types(text: str) -> set:
    q = " " + _final_norm(text, False) + " "
    out = set()
    if not q.strip():
        return out
    for typ, words in _FINAL_TYPE_SYNONYMS.items():
        for word in words:
            w = _final_norm(word, False)
            if w and re.search(rf"(?<!\w){re.escape(w)}(?!\w)", q):
                out.add(typ)
                break
    if "cleanser" in out:
        out.discard("gel")
        out.discard("cream")
        out.discard("moisturizer")
    return out


def detect_product_types(text: str) -> set:
    return _final_detect_types(text)
...
def product_types(item: dict) -> set:
    return _final_detect_types(product_identity_text(item))


def _final_has_type_conflict(query_types: set, item_types: set) -> bool:
    for group in _FINAL_CONFLICTS:
        if group & query_types and group & item_types and not (query_types & item_types & group):
            return True
    return False


def has_type_conflict(query_types: set, item_types: set) -> bool:
    return _final_has_type_conflict(query_types, item_types)


def _final_types_compatible(query_types: set, item_types: set, allow_close: bool = False) -> bool:
    if not query_types or not item_types:
        return True
    if _final_has_type_conflict(query_types, item_types):
        return False
    q_cos = query_types & COSMETIC_TYPES
    i_cos = item_types & COSMETIC_TYPES
    if q_cos and i_cos:
        if q_cos & i_cos:
            return True
        if allow_close and (q_cos <= {"lotion", "moisturizer"} and i_cos <= {"lotion", "moisturizer"}):
            return True
        return False
    return True


def _final_terms_for_item(item: dict) -> List[Tuple[str, str, str]]:
    terms: List[Tuple[str, str, str]] = []
    name = str(item.get("name", "") or "").strip()
    if name:
        terms.append(("name", name, _final_norm(name, False)))
    for alias in split_aliases(str(item.get("aliases", "") or "")):
        alias_norm = _final_norm(alias, False)
        if not alias_norm or alias_norm in _FINAL_BRAND_ONLY or is_weak_only_query(alias_norm):
            continue
        terms.append(("alias", alias, alias_norm))
    out: List[Tuple[str, str, str]] = []
    seen = set()
    for kind, raw, norm in terms:
        if norm and norm not in seen:
            out.append((kind, raw, norm))
            seen.add(norm)
    return out


def _final_meaningful_fuzzy(q: str) -> bool:
    tokens = _final_tokens(q)
    if len(tokens) < 2 or is_weak_only_query(q):
        return False
    return any(t not in _FINAL_WEAK_WORDS or t in _FINAL_IMPORTANT_WORDS for t in tokens)


def _final_required_terms_present(q: str, item: dict) -> bool:
    tokens = _final_tokens(q)
    important = [t for t in tokens if t not in _FINAL_WEAK_WORDS or t in _FINAL_IMPORTANT_WORDS]
    if not important:
        return False
    hay = _final_norm(product_identity_text(item), False)
    return all(t in hay for t in important)


def resolve_product_query(query: str, source: str = "text") -> Optional[dict]:
    q = _final_norm(extract_product_query(query or "") or query or "", True)
    if not q:
        return None
    products = load_products()

    for item in products:
        if q == _final_norm(item.get("name", ""), False):
            return item

    qtypes = _final_detect_types(q)
    for item in products:
        itypes = product_types(item)
        if not _final_types_compatible(qtypes, itypes):
            continue
        for kind, raw, norm in _final_terms_for_item(item):
            if kind == "alias" and q == norm:
                return item
        ...
        for item in products:
            ...
            for kind, raw, norm in _final_terms_for_item(item):
                if len(q) >= 4 and q in norm:
                    phrase_matches.append((len(q) / max(len(norm), 1), item))
                    break
    if phrase_matches:
        phrase_matches.sort(key=lambda x: x[0], reverse=True)
        return phrase_matches[0][1]

    if not _final_meaningful_fuzzy(q):
        return None
    ...
    for item in products:
        ...
        if not _final_types_compatible(qtypes, itypes):
            continue
        if not _final_required_terms_present(q, item):
            continue
        best = 0.0
        for kind, raw, norm in _final_terms_for_item(item):
            best = max(best, _PB_SEQUENCE_MATCHER(None, q, norm).ratio())
        if best >= 0.94:
            fuzzy.append((best, item))
    fuzzy.sort(key=lambda x: x[0], reverse=True)
    if not fuzzy:
...
def exact_safe_product_match(text: str) -> Optional[dict]:
    return resolve_product_query(text)
...
def query_product_matches(text: str, limit: int = 8) -> List[dict]:
    item = resolve_product_query(text)
    return [item] if item else []


def cosmetic_alternatives_for_query(text: str, limit: int = 6) -> List[dict]:
    q = _final_norm(text or "", True)
    qtypes = _final_detect_types(q)
    if not (qtypes & COSMETIC_TYPES):
        return []
    allowed = set(qtypes & COSMETIC_TYPES)
    if "lotion" in allowed:
        allowed.add("moisturizer")
    if "moisturizer" in allowed:
        allowed.add("lotion")
    resolved = resolve_product_query(q)
    resolved_key = _final_norm(resolved.get("name", ""), False) if resolved else ""
    tokens = [t for t in _final_tokens(q) if t not in _FINAL_WEAK_WORDS or t in _FINAL_BRAND_ONLY]
    scored: List[Tuple[int, dict]] = []
    seen = set()
    for item in load_products():
        key = _final_norm(item.get("name", ""), False)
        if not key or key == resolved_key or key in seen or not is_product_available(item):
            continue
        itypes = product_types(item)
        if not (itypes & allowed):
            continue
        if not _final_types_compatible(qtypes, itypes, allow_close=True):
            continue
        hay = _final_norm(product_identity_text(item), False)
        score = sum(1 for t in tokens if t in hay)
        if any(b in q and b in hay for b in _FINAL_BRAND_ONLY):
            score += 4
        score += len(itypes & allowed) * 2
...
def suggested_products(text: str, limit: int = 4) -> List[dict]:
    if is_weak_only_query(text):
        return []
    return cosmetic_alternatives_for_query(text, limit)
...
def build_unavailable_reply(text: str, from_number: str = "") -> str:
    ...
    if exact:
        if from_number:
            ...
            PENDING_ALT_REQUESTS.pop(from_number, None)
        return build_product_reply(exact, text)
    alts = cosmetic_alternatives_for_query(text, limit=8)
    if from_number:
        PENDING_ALT_REQUESTS.pop(from_number, None)
    if alts:
        return build_cosmetic_alternatives_direct_reply(text, alts, from_number)
...
def product_from_vision(data: dict) -> Optional[dict]:
    names: List[str] = []
    for key in ["product_name", "product_names", "visible_text", "matched_product_names"]:
        val = data.get(key)
        if isinstance(val, str) and val.strip():
            names.append(val.strip())
        elif isinstance(val, list):
            names.extend([str(x).strip() for x in val if str(x).strip()])
    brand = str(data.get("brand", "") or "").strip()
    product_type = str(data.get("type", "") or data.get("product_type", "") or "").strip()
    size = str(data.get("size", "") or "").strip()
    candidates = []
    for name in names:
        candidates.append(" ".join([brand, name, product_type, size]).strip())
        candidates.append(" ".join([brand, name, product_type]).strip())
        candidates.append(" ".join([brand, name]).strip())
        candidates.append(name)
    seen = set()
    for candidate in candidates:
        key = _final_norm(candidate, True)
        if not key or key in seen:
            continue
        seen.add(key)
        item = resolve_product_query(candidate, source="image")
        if item:
            return item
    return None
...
def suggestions_from_ai(ai_data: dict, user_text: str, limit: int = 4) -> List[dict]:
    ...
    for name in names:
        ...
        if len(out) >= limit:
            return out
    for item in suggested_products(user_text, limit=limit):
        key = _final_norm(item.get("name", ""), False)
        if key and key not in seen:
            out.append(item)
            seen.add(key)
...
def _vision_prompt() -> str:
    return (
        "Return JSON only. Read the product package text. Do not guess price or availability. "
        "Fields only: brand, product_name, type, size, confidence, clarity. "
        "type must be one of: cleanser, lotion, moisturizer, cream, serum, sunscreen, toner, gel, oil, shampoo, tablets, capsules, syrup, injection, unknown. "
        "clarity must be good, medium, or bad. "
        "{\"brand\":\"\",\"product_name\":\"\",\"type\":\"\",\"size\":\"\",\"confidence\":0.0,\"clarity\":\"good|medium|bad\"}"
    )


# =============================================================================
# PRICEBOT_FINAL_SAFE_MATCH_V3
# Authoritative matching/vision pipeline: no category-only matches, no brand/type
# guessing, and no silent replies.
# =============================================================================

import time as _pb_time

_PB_V3_SEQUENCE_MATCHER = __import__("difflib").SequenceMatcher
_PB_V3_INDEX_CACHE = {"fingerprint": None, "built_at": 0.0, "entries": []}

_PB_V3_BRANDS = {
    "cerave": ["cerave", "cera ve", "سيرافي"],
    "la roche": ["la roche", "laroche", "la roche posay", "لاروش", "لاروش بوزيه"],
    "vichy": ["vichy", "فيشي"],
    "bioderma": ["bioderma", "بيوديرما"],
    "eucerin": ["eucerin", "يوسرين"],
    "acm": ["acm"],
    "svr": ["svr"],
    "uriage": ["uriage", "يورياج"],
    "avene": ["avene", "avène", "افين"],
    "alfa": ["alfa", "alpha", "الفا"],
}

_PB_V3_REQUEST_WORDS = {
    "متوفر", "متوفرة", "عندكم", "موجود", "موجودة", "السعر", "سعر", "بكم", "كم",
    "هل", "نبي", "ابي", "أبي", "اريد", "أريد", "لو سمحت", "من فضلك",
    "available", "price", "do you have", "how much", "have", "need", "want", "please",
}
...
_PB_V3_TYPE_SYNONYMS = {
    "cleanser": ["cleanser", "cleansing", "wash", "foaming", "gel moussant", "lavante", "effaclar", "cleanance", "cleanser gel", "منظف", "غسول"],
    "body_wash": ["body wash", "bodywash", "shower gel", "غسول جسم", "جسم"],
    "shampoo": ["shampoo", "hair wash", "شامبو", "شعر"],
    "lotion": ["lotion", "لوشن"],
    "moisturizer": ["moisturizer", "moisturizing", "moisturising", "moisturiser", "hydrating", "مرطب"],
    "cream": ["cream", "كريم"],
    "serum": ["serum", "سيروم"],
    "sunscreen": ["sunscreen", "sun screen", "spf", "واقي شمس"],
    "toner": ["toner", "تونر"],
...
_PB_V3_AREA_SYNONYMS = {
    "face": ["face", "facial", "visage", "وجه", "الوجه", "بشرة", "البشرة", "oily skin", "dry skin"],
    "body": ["body", "body wash", "جسم", "للجسم"],
    "baby": ["baby", "kids", "child", "infant", "بيبي", "اطفال", "أطفال", "طفل", "baby shark"],
    "hair": ["hair", "shampoo", "scalp", "شعر", "فروة"],
}

_PB_V3_TYPE_SYNONYMS["shampoo"] = ["shampoo", "hair wash", "شامبو", "شعر"]

_PB_V3_WEAK_WORDS = {
    "cerave", "vichy", "bioderma", "eucerin", "acm", "svr", "uriage", "avene", "la", "roche",
    "cleanser", "lotion", "cream", "serum", "gel", "wash", "skin", "daily", "hydrating",
    "moisturizing", "moisturising", "moisturizer", "body", "face", "baby", "shampoo",
}

_PB_V3_IMPORTANT_WORDS = {
    "sa", "baume", "effaclar", "toleriane", "foaming", "hydrating", "moisturizing",
    "sunscreen", "spf", "oil", "cleanance", "acne", "rough", "bumpy", "salicylic",
}

_PB_V3_COSMETIC_TYPES = {
    "cleanser", "lotion", "moisturizer", "cream", "serum", "sunscreen", "toner", "gel",
    "oil", "shampoo", "body_wash",
}


def _pb_v3_log(event: str, data: dict) -> None:
    try:
        print("PRICEBOT_MATCH " + json.dumps({"event": event, **(data or {})}, ensure_ascii=False, default=str), flush=True)
    except Exception as exc:
        print(f"PRICEBOT_MATCH_LOG_ERROR {event}: {exc}", flush=True)


def _pb_v3_pharmacy_name() -> str:
    name = get_config("PHARMACY_NAME", "").strip()
    if not name or "طµ" in name or "ظ" in name:
        return "صيدلية بدر البشرية"
    return name


def _pb_v3_header() -> str:
    return f"🌿 {_pb_v3_pharmacy_name()}"


def _pb_v3_basic_norm(text: str) -> str:
    s = str(text or "").strip().lower()
    if not s:
        return ""
    for src, dst in {
...
def _pb_v3_norm(text: str, strip_request_words: bool = False) -> str:
    s = _pb_v3_basic_norm(text)
    if not s:
        return ""
    for src, dst in sorted(_PB_V3_SYNONYMS.items(), key=lambda x: len(x[0]), reverse=True):
        src_norm = _pb_v3_basic_norm(src)
        if src_norm:
            s = re.sub(rf"(?<!\w){re.escape(src_norm)}(?!\w)", dst, s)
    s = re.sub(r"\bcera\s+ve\b", "cerave", s)
    s = re.sub(r"\bla\s+roche\b", "la roche", s)
    if strip_request_words:
        for phrase in sorted(_PB_V3_REQUEST_WORDS, key=len, reverse=True):
            p = _pb_v3_norm(phrase, False)
            if p:
                s = re.sub(rf"(?<!\w){re.escape(p)}(?!\w)", " ", s)
    return re.sub(r"\s+", " ", s).strip()


def _pb_v3_tokens(text: str) -> List[str]:
    return [t for t in _pb_v3_norm(text, False).split() if t]


def _pb_v3_detect_brand(text: str) -> str:
    q = " " + _pb_v3_norm(text, False) + " "
    for brand, aliases in _PB_V3_BRANDS.items():
        for alias in aliases:
            a = _pb_v3_norm(alias, False)
            if a and re.search(rf"(?<!\w){re.escape(a)}(?!\w)", q):
                return brand
    return ""


def _pb_v3_detect_types(text: str) -> set:
    q = " " + _pb_v3_norm(text, False) + " "
    out = set()
    for typ, words in _PB_V3_TYPE_SYNONYMS.items():
        for word in words:
            w = _pb_v3_norm(word, False)
            if w and re.search(rf"(?<!\w){re.escape(w)}(?!\w)", q):
                out.add(typ)
                break
    if "body_wash" in out:
        out.add("cleanser")
    if "shampoo" in out and "cleanser" in out and "body_wash" not in out:
        out.discard("cleanser")
    return out


def _pb_v3_detect_areas(text: str) -> set:
    q = " " + _pb_v3_norm(text, False) + " "
    out = set()
    for area, words in _PB_V3_AREA_SYNONYMS.items():
        for word in words:
            w = _pb_v3_norm(word, False)
            if w and re.search(rf"(?<!\w){re.escape(w)}(?!\w)", q):
                out.add(area)
...
def _pb_v3_product_text(item: dict) -> str:
    return " ".join(str(item.get(k, "") or "") for k in [
        "name", "aliases", "active_ingredient", "brand", "company", "form", "strength", "pack", "notes"
    ])


def _pb_v3_strong_tokens(text: str, brand: str = "", types: Optional[set] = None, areas: Optional[set] = None) -> List[str]:
    types = types or set()
    areas = areas or set()
    brand_parts = set(_pb_v3_tokens(brand))
    type_words = set()
    for typ in types:
        type_words.add(_pb_v3_norm(typ, False))
        for w in _PB_V3_TYPE_SYNONYMS.get(typ, []):
            type_words.update(_pb_v3_tokens(w))
    area_words = set()
    for area in areas:
        area_words.add(_pb_v3_norm(area, False))
        for w in _PB_V3_AREA_SYNONYMS.get(area, []):
            area_words.update(_pb_v3_tokens(w))
    out = []
    seen = set()
    for token in _pb_v3_tokens(text):
        if token in seen or token in brand_parts:
            continue
        if token in type_words and token not in _PB_V3_IMPORTANT_WORDS:
            continue
        if token in area_words and token not in _PB_V3_IMPORTANT_WORDS:
            continue
        if token in _PB_V3_WEAK_WORDS and token not in _PB_V3_IMPORTANT_WORDS:
            continue
...
def _pb_v3_parse_query(query: str, source: str = "text") -> dict:
    cleaned = extract_product_query(query or "") if "extract_product_query" in globals() else (query or "")
    norm = _pb_v3_norm(cleaned or query or "", True)
    brand = _pb_v3_detect_brand(norm)
    types = _pb_v3_detect_types(norm)
    areas = _pb_v3_detect_areas(norm)
    if "cleanser" in types and not areas and source == "image":
        areas.add("face")
    size_match = re.search(r"\b\d+(?:\.\d+)?\s*(?:ml|ملي|g|gm|oz|fl oz|mg|spf)\b", norm)
    strong = _pb_v3_strong_tokens(norm, brand, types, areas)
    category_tokens = [t for t in _pb_v3_tokens(norm) if t in _PB_V3_WEAK_WORDS]
    weak_category_only = not strong and bool(category_tokens or brand or types)
    return {
        "raw": query or "",
        "normalized": norm,
        "brand": brand,
        ...
        "weak_category_only": weak_category_only,
        "source": source,
    }


def _pb_v3_item_terms(item: dict) -> List[Tuple[str, str, str]]:
    terms = []
    name = str(item.get("name", "") or "").strip()
    if name:
        terms.append(("name", name, _pb_v3_norm(name, False)))
    for alias in split_aliases(str(item.get("aliases", "") or "")):
        a = _pb_v3_norm(alias, False)
        if not a:
            continue
        if a in {b for b in _PB_V3_BRANDS} or all(t in _PB_V3_WEAK_WORDS for t in _pb_v3_tokens(a)):
            continue
        terms.append(("alias", alias, a))
    out = []
    seen = set()
    for kind, raw, norm in terms:
        if norm and norm not in seen:
...
def _pb_v3_products_index() -> List[dict]:
    products = load_products()
    fingerprint = (
        len(products),
        sum(len(str(p.get("name", "") or "")) for p in products),
        str(products[0].get("name", "") if products else ""),
        str(products[-1].get("name", "") if products else ""),
    )
    now = _pb_time.time()
    if _PB_V3_INDEX_CACHE.get("fingerprint") == fingerprint and now - float(_PB_V3_INDEX_CACHE.get("built_at", 0.0)) < 60:
        return list(_PB_V3_INDEX_CACHE.get("entries") or [])
    entries = []
    for item in products:
        text = _pb_v3_product_text(item)
        name = str(item.get("name", "") or "")
        entries.append({
            "item": item,
            "name_norm": _pb_v3_norm(name, False),
            "terms": _pb_v3_item_terms(item),
            "text_norm": _pb_v3_norm(text, False),
            "brand": _pb_v3_detect_brand(text),
            "types": _pb_v3_detect_types(text),
            "areas": _pb_v3_detect_areas(text),
            "tokens": set(_pb_v3_tokens(text)),
        })
    _PB_V3_INDEX_CACHE.update({"fingerprint": fingerprint, "built_at": now, "entries": entries})
    _pb_v3_log("INDEX_BUILT", {"count": len(entries)})
    return entries


def _pb_v3_brand_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    qbrand = info.get("brand", "")
    if not qbrand:
        return True, ""
    item_brand = entry.get("brand") or _pb_v3_detect_brand(entry.get("text_norm", ""))
    if item_brand and item_brand != qbrand:
        return False, "brand_conflict"
    if not item_brand and qbrand not in entry.get("text_norm", ""):
        return False, "brand_conflict"
    return True, ""


def _pb_v3_type_ok(info: dict, entry: dict, close_lotion: bool = True) -> Tuple[bool, str]:
    qtypes = set(info.get("types") or set())
    itypes = set(entry.get("types") or set())
    if not qtypes or not itypes:
        return True, ""
    if "cleanser" in qtypes:
        if "serum" in itypes or "lotion" in itypes or "cream" in itypes or "moisturizer" in itypes or "sunscreen" in itypes:
            return False, "type_conflict"
        if ("shampoo" in itypes or "body_wash" in itypes) and not (info.get("areas") & {"hair", "body", "baby"}):
            return False, "type_conflict"
        return ("cleanser" in itypes, "" if "cleanser" in itypes else "type_conflict")
    if "serum" in qtypes and "serum" not in itypes:
        return False, "type_conflict"
    if "lotion" in qtypes:
        if "serum" in itypes or "cleanser" in itypes:
            return False, "type_conflict"
        if close_lotion and ("lotion" in itypes or "moisturizer" in itypes):
            return True, ""
        return ("lotion" in itypes, "" if "lotion" in itypes else "type_conflict")
    if "sunscreen" in qtypes and "sunscreen" not in itypes:
        return False, "type_conflict"
    if "shampoo" in qtypes and "shampoo" not in itypes:
        return False, "type_conflict"
    for qtype in qtypes:
        if qtype in {"tablets", "capsules", "syrup", "injection"} and qtype not in itypes:
            return False, "type_conflict"
    return True, ""


def _pb_v3_area_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    qareas = set(info.get("areas") or set())
    iareas = set(entry.get("areas") or set())
    qtypes = set(info.get("types") or set())
    if "cleanser" in qtypes and not qareas:
        if iareas & {"baby", "body", "hair"}:
            return False, "area_conflict"
    if qareas:
        if "face" in qareas and iareas & {"baby", "body", "hair"}:
            return False, "area_conflict"
        if "body" in qareas and iareas and "body" not in iareas:
            return False, "area_conflict"
        if "hair" in qareas and iareas and "hair" not in iareas:
            return False, "area_conflict"
        if "baby" in qareas and iareas and "baby" not in iareas:
            return False, "area_conflict"
    return True, ""


def _pb_v3_candidate_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    for check in (_pb_v3_brand_ok, _pb_v3_type_ok, _pb_v3_area_ok):
        ok, reason = check(info, entry)
        if not ok:
            return False, reason
    return True, ""


def _pb_v3_is_weak_category_only(query: str) -> bool:
    info = _pb_v3_parse_query(query)
    return bool(info.get("weak_category_only"))


def _pb_v3_resolve_product_query(query: str, source: str = "text") -> Optional[dict]:
    info = _pb_v3_parse_query(query, source)
    q = info["normalized"]
    rejected = []
    top = []
    if not q:
        _pb_v3_log("FINAL_DECISION", {"RAW_QUERY": query, "SOURCE": source, "FINAL_DECISION": "fallback"})
        return None

    entries = _pb_v3_products_index()
    _pb_v3_log("QUERY", {
        "RAW_QUERY": info["raw"],
        "NORMALIZED_QUERY": q,
        "SOURCE": source,
        "DETECTED_BRAND": info["brand"],
        "DETECTED_TYPE": sorted(info["types"]),
        "DETECTED_AREA": sorted(info["areas"]),
        "STRONG_TOKENS": info["strong_tokens"],
    })
    ...
    for entry in entries:
        if q == entry["name_norm"]:
            item = entry["item"]
            _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "exact_name"})
            return item

    # 2) exact alias match.
    for entry in entries:
        ok, reason = _pb_v3_candidate_ok(info, entry)
        if not ok:
            rejected.append({"name": entry["item"].get("name", ""), "reason": reason})
            continue
        for kind, raw, norm in entry["terms"]:
            if kind == "alias" and q == norm:
                item = entry["item"]
                _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "exact_alias"})
                return item

    if info.get("weak_category_only"):
        _pb_v3_log("FINAL_DECISION", {
            "FINAL_DECISION": "unavailable",
            "REJECT_REASON": "weak_category_only",
            "CANDIDATES_TOP_5": top[:5],
            "REJECTED": rejected[:10],
        })
        return None

    # 3) full phrase contains match, only for meaningful phrases.
    if len(q) >= 5:
        phrase_matches = []
        for entry in entries:
            ok, reason = _pb_v3_candidate_ok(info, entry)
            ...
            for kind, raw, norm in entry["terms"]:
                if q in norm:
                    score = len(q) / max(len(norm), 1)
                    phrase_matches.append((score, entry))
                    top.append({"name": entry["item"].get("name", ""), "score": round(score, 3), "stage": "phrase"})
                    break
        if phrase_matches:
            phrase_matches.sort(key=lambda x: x[0], reverse=True)
            item = phrase_matches[0][1]["item"]
            _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "phrase", "CANDIDATES_TOP_5": top[:5]})
            return item

    # 4) strict token match: requires brand or enough strong product tokens.
    strong = list(info.get("strong_tokens") or [])
    if not info.get("brand") and len(strong) < 2:
        _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "unavailable", "REJECT_REASON": "not_enough_strong_tokens", "REJECTED": rejected[:10]})
        return None
    ...
    for entry in entries:
        ...
        missing = [t for t in strong if t not in entry_tokens]
        if missing:
            rejected.append({"name": entry["item"].get("name", ""), "reason": "missing_strong_tokens", "missing": missing[:4]})
            continue
        score = 0.70 + (0.08 * len(strong))
        if info.get("brand"):
            score += 0.10
        if info.get("types") & entry.get("types", set()):
            score += 0.05
        token_matches.append((min(score, 0.98), entry))
        top.append({"name": entry["item"].get("name", ""), "score": round(min(score, 0.98), 3), "stage": "tokens"})

    if token_matches:
        token_matches.sort(key=lambda x: x[0], reverse=True)
        if len(token_matches) == 1 or token_matches[0][0] - token_matches[1][0] >= 0.04:
            item = token_matches[0][1]["item"]
            _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "tokens", "CANDIDATES_TOP_5": top[:5]})
            return item
        rejected.append({"name": token_matches[0][1]["item"].get("name", ""), "reason": "ambiguous_close_scores"})

    # 5) strict fuzzy on filtered candidates only.
    fuzzy = []
    for entry in entries:
        ok, reason = _pb_v3_candidate_ok(info, entry)
        if not ok:
            continue
        if strong and any(t not in set(entry.get("tokens") or set()) for t in strong):
            continue
        best = 0.0
        for kind, raw, norm in entry["terms"]:
            best = max(best, _PB_V3_SEQUENCE_MATCHER(None, q, norm).ratio())
        if best >= 0.95:
            fuzzy.append((best, entry))
            top.append({"name": entry["item"].get("name", ""), "score": round(best, 3), "stage": "fuzzy"})
        else:
            rejected.append({"name": entry["item"].get("name", ""), "reason": "low_score", "score": round(best, 3)})
    fuzzy.sort(key=lambda x: x[0], reverse=True)
    if fuzzy and (len(fuzzy) == 1 or fuzzy[0][0] - fuzzy[1][0] >= 0.04):
        item = fuzzy[0][1]["item"]
        _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "fuzzy", "CANDIDATES_TOP_5": top[:5]})
        return item

    _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "unavailable", "CANDIDATES_TOP_5": top[:5], "REJECTED": rejected[:10]})
    return None


def _pb_v3_is_product_available(item: dict) -> bool:
    status = _pb_v3_norm(str(item.get("available", "متوفر") or "متوفر"), False)
    unavailable = {"غير متوفر", "غير موجود", "نفذ", "ناقص", "لا", "no", "0"}
    return status not in unavailable


def _pb_v3_format_price(price: str) -> str:
...
def _pb_v3_build_product_reply(item: dict, original_text: str = "") -> str:
    available = str(item.get("available", "") or "متوفر").strip()
    price = _pb_v3_format_price(item.get("price", ""))
    lines = [_pb_v3_header(), "", f"✅ المنتج: {str(item.get('name', '')).strip()}", f"📦 الحالة: {available}"]
    if price:
        lines.append(f"💰 السعر: {price}")
    lines += ["", "للحجز اكتب: نعم"]
    return "\n".join(lines)
...
def _pb_v3_allowed_alternative_types(info: dict) -> set:
    qtypes = set(info.get("types") or set())
    if "cleanser" in qtypes:
        return {"cleanser"}
    if "lotion" in qtypes or "moisturizer" in qtypes:
        return {"lotion", "moisturizer"}
    if "serum" in qtypes:
        return {"serum"}
    if "sunscreen" in qtypes:
        return {"sunscreen"}
    if "cream" in qtypes:
        return {"cream", "moisturizer"}
    if "toner" in qtypes:
        return {"toner"}
    if "gel" in qtypes:
        return {"gel"}
...
def _pb_v3_cosmetic_alternatives(text: str, limit: int = 6) -> List[dict]:
    info = _pb_v3_parse_query(text)
    allowed = _pb_v3_allowed_alternative_types(info)
    if not allowed:
        return []
    entries = _pb_v3_products_index()
    qbrand = info.get("brand", "")
    qareas = set(info.get("areas") or set())
    if "cleanser" in allowed and not qareas:
        qareas.add("face")
    scored = []
    seen = set()
    for entry in entries:
        item = entry["item"]
        key = entry["name_norm"]
        if not key or key in seen or not _pb_v3_is_product_available(item):
            continue
        itypes = set(entry.get("types") or set())
        iareas = set(entry.get("areas") or set())
        if not (itypes & allowed):
            continue
        if "cleanser" in allowed and (itypes & {"shampoo", "body_wash"} or iareas & {"baby", "body", "hair"}):
            continue
        if qareas:
            fake_info = dict(info)
            fake_info["areas"] = qareas
            ok, _ = _pb_v3_area_ok(fake_info, entry)
            if not ok:
                continue
        score = 0
        if qbrand and entry.get("brand") == qbrand:
            score += 5
        score += len(itypes & allowed) * 3
        entry_tokens = set(entry.get("tokens") or set())
        for token in info.get("strong_tokens") or []:
            if token in entry_tokens:
                score += 1
        scored.append((score, item))
        seen.add(key)
    scored.sort(key=lambda x: x[0], reverse=True)
    alts = [item for score, item in scored[:limit]]
    _pb_v3_log("FINAL_DECISION", {"FINAL_DECISION": "alternatives" if alts else "unavailable", "ALTERNATIVES": [a.get("name", "") for a in alts[:5]]})
    return alts


def _pb_v3_build_alternatives_reply(text: str, alts: List[dict], from_number: str = "") -> str:
    alts = [a for a in alts if _pb_v3_is_product_available(a)]
    if from_number and alts:
        PENDING_OPTIONS[from_number] = {"items": alts[:8], "reason": "cosmetic_alternatives"}
    if not alts:
        return _pb_v3_build_unavailable_reply(text, from_number, allow_alts=False)
    best = alts[0]
    lines = [
        _pb_v3_header(),
        "",
        ...
        "",
        "⭐ أنسب بديل مقترح:",
        str(best.get("name", "")).strip(),
    ]
    price = _pb_v3_format_price(best.get("price", ""))
    if price:
        lines.append(f"السعر: {price}")
    lines.append("")
    for i, item in enumerate(alts[:5], 1):
        suffix = _pb_v3_format_price(item.get("price", ""))
        lines.append(f"{i}) {str(item.get('name', '')).strip()}" + (f" - {suffix}" if suffix else ""))
    lines += ["", "اكتب رقم البديل المطلوب للحجز أو الاستفسار."]
    return "\n".join(lines)


def _pb_v3_build_unavailable_reply(text: str, from_number: str = "", allow_alts: bool = True) -> str:
    exact = _pb_v3_resolve_product_query(text)
    if exact:
        if from_number:
            LAST_PRODUCT[from_number] = exact
        return _pb_v3_build_product_reply(exact, text)
    if allow_alts:
        alts = _pb_v3_cosmetic_alternatives(text, limit=6)
        if alts:
            return _pb_v3_build_alternatives_reply(text, alts, from_number)
    return "\n".join([
        _pb_v3_header(),
        "",
        "المنتج المطلوب غير متوفر حالياً في قائمة الصيدلية.",
        "يمكنك إرسال اسم منتج آخر أو صورة أوضح.",
    ])


def _pb_v3_build_unclear_image_reply() -> str:
    return f"{_pb_v3_header()}\n\nالصورة غير واضحة بما يكفي للتعرف على المنتج. الرجاء إرسال صورة أوضح أو كتابة اسم المنتج."


def _pb_v3_build_image_timeout_reply() -> str:
    return f"{_pb_v3_header()}\n\nالصورة أخذت وقتاً أطول من المعتاد. الرجاء إرسال صورة أوضح أو كتابة اسم المنتج."


def _pb_v3_build_fallback_reply(text: str = "") -> str:
    return f"{_pb_v3_header()}\n\nأرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."


def _pb_v3_build_welcome_reply() -> str:
    return f"{_pb_v3_header()}\n\nأرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."


def _pb_v3_product_from_vision(data: dict) -> Optional[dict]:
    brand = str(data.get("brand", "") or "").strip()
    product_name = str(data.get("product_name", "") or "").strip()
    if not product_name and isinstance(data.get("product_names"), list):
        product_name = " ".join(str(x) for x in data.get("product_names") if str(x).strip())
    product_type = str(data.get("product_type", "") or data.get("type", "") or "").strip()
    target_area = str(data.get("target_area", "") or "").strip()
    size = str(data.get("size", "") or "").strip()
    candidates = [
        " ".join([brand, product_name]).strip(),
        " ".join([brand, product_name, product_type]).strip(),
        " ".join([brand, product_name, product_type, target_area]).strip(),
        " ".join([brand, product_name, product_type, target_area, size]).strip(),
        product_name,
    ]
    seen = set()
    for query in candidates:
        key = _pb_v3_norm(query, True)
        if not key or key in seen:
            continue
        seen.add(key)
        item = _pb_v3_resolve_product_query(query, source="image")
        if item:
            return item
    return None


def _pb_v3_process_image_message(msg: dict, from_number: str) -> str:
    start_ts = datetime.now()
    image = msg.get("image", {}) or {}
    media_id = image.get("id", "")
    caption = image.get("caption", "") or ""
    try:
        send_whatsapp_message(from_number, f"{_pb_v3_header()}\n\n🌿 جاري فحص الصورة، انتظر لحظات...")
    except Exception as exc:
        print("IMAGE ACK SEND SKIP:", exc, flush=True)
    ...
    if not media_sha and media_bytes:
        media_sha = hashlib.sha256(media_bytes).hexdigest()
    if not media_bytes:
        _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unclear", "REJECT_REASON": "media_download_failed"})
        return _pb_v3_build_unclear_image_reply()

    vision = call_gemini_vision_json(media_bytes, mime_type) if image_ai_enabled() else {}
    _pb_v3_log("AI_PARSED_JSON", {"SOURCE": "image", "AI_PARSED_JSON": vision})
    image_type = str(vision.get("image_type", "") or "").lower() if isinstance(vision, dict) else ""
    if image_type == "timeout":
        _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "timeout"})
        return _pb_v3_build_image_timeout_reply()

    confidence = float(vision.get("confidence") or 0.0) if isinstance(vision, dict) else 0.0
    clarity = str(vision.get("clarity", "") or "").lower() if isinstance(vision, dict) else ""
    if not vision or clarity in {"bad", "unclear", "غير واضح"} or confidence < 0.65:
        _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unclear", "confidence": confidence, "clarity": clarity})
        return _pb_v3_build_unclear_image_reply()

    brand = str(vision.get("brand", "") or "").strip()
    product_name = str(vision.get("product_name", "") or "").strip()
    if not product_name and isinstance(vision.get("product_names"), list):
        product_name = " ".join(str(x) for x in vision.get("product_names") if str(x).strip())
    product_type = str(vision.get("product_type", "") or vision.get("type", "") or "").strip()
    target_area = str(vision.get("target_area", "") or "").strip()
    size = str(vision.get("size", "") or "").strip()
    internal_query = " ".join([brand, product_name, product_type, target_area, size]).strip()
    if not internal_query:
        _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unclear", "REJECT_REASON": "empty_identity"})
        return _pb_v3_build_unclear_image_reply()

    item = _pb_v3_product_from_vision(vision)
    if item:
        LAST_PRODUCT[from_number] = item
        _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "seconds": (datetime.now() - start_ts).total_seconds()})
        return _pb_v3_build_product_reply(item, caption)

    _pb_v3_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unavailable", "INTERNAL_QUERY": internal_query})
    return _pb_v3_build_unavailable_reply(internal_query, from_number)


def _pb_v3_is_greeting(text: str) -> bool:
    q = _pb_v3_norm(text, False)
    return q in {"السلام عليكم", "سلام", "مرحبا", "اهلا", "اهلين", "hi", "hello", "test"} or q.startswith("السلام")


def _pb_v3_build_reply(text: str, from_number: str = "") -> str:
    raw = text or ""
    q = _pb_v3_norm(raw, True)
    _pb_v3_log("RAW_MESSAGE", {"RAW_QUERY": raw, "NORMALIZED_QUERY": q, "SOURCE": "text"})
    try:
        if from_number and (from_number in PENDING_OPTIONS or from_number in PENDING_SUGGESTION or from_number in LAST_PRODUCT):
            if is_positive_response_query(q) or is_negative_response_query(q) or re.fullmatch(r"[0-9٠-٩]{1,2}", str(raw).strip()):
                return _PB_V3_OLD_BUILD_REPLY(raw, from_number)
    except Exception:
        pass
    if _pb_v3_is_greeting(raw):
        return _pb_v3_build_welcome_reply()
    try:
        if is_available_catalog_request(raw):
            return build_available_products_reply()
    except Exception:
        pass
    try:
        corrected = memory_get_admin_correction(raw)
        if corrected:
            LAST_PRODUCT[from_number] = corrected
            return _pb_v3_build_product_reply(corrected, raw)
    except Exception:
        pass
    try:
        if asks_medical_advice(raw):
            return build_medical_safety_reply(raw)
    except Exception:
        pass
    item = _pb_v3_resolve_product_query(raw, source="text")
    if item:
        if from_number:
            LAST_PRODUCT[from_number] = item
        return _pb_v3_build_product_reply(item, raw)
    info = _pb_v3_parse_query(raw, "text")
    if info.get("brand") or info.get("types") or len(info.get("strong_tokens") or []) >= 2:
        return _pb_v3_build_unavailable_reply(raw, from_number)
    return _pb_v3_build_fallback_reply(raw)


def _pb_v3_vision_prompt() -> str:
    return (
        "Return JSON only. Read the product package. Do not decide price or availability. "
        "Use fields only: brand, product_name, product_type, target_area, size, confidence, clarity. "
        "product_type examples: face cleanser, cleanser, lotion, moisturizer, cream, serum, sunscreen, toner, gel, oil, shampoo. "
        "target_area examples: face, body, baby, hair, unknown. clarity: good or unclear. "
        "{\"brand\":\"\",\"product_name\":\"\",\"product_type\":\"\",\"target_area\":\"\",\"size\":\"\",\"confidence\":0.0,\"clarity\":\"good|unclear\"}"
    )


def _pb_v3_install_authoritative() -> None:
    global _PB_V3_SEQUENCE_MATCHER
    _PB_V3_SEQUENCE_MATCHER = __import__("difflib").SequenceMatcher
    globals()["normalize_product_query"] = lambda text, remove_request_words=True: _pb_v3_norm(text, strip_request_words=remove_request_words)
    globals()["detect_product_types"] = _pb_v3_detect_types
    globals()["product_types"] = lambda item: _pb_v3_detect_types(_pb_v3_product_text(item))
    globals()["product_identity_text"] = _pb_v3_product_text
    globals()["is_weak_only_query"] = _pb_v3_is_weak_category_only
    globals()["resolve_product_query"] = _pb_v3_resolve_product_query
    globals()["exact_safe_product_match"] = lambda text: _pb_v3_resolve_product_query(text)
    globals()["find_product"] = lambda text: _pb_v3_resolve_product_query(text)
    globals()["query_product_matches"] = lambda text, limit=8: ([_pb_v3_resolve_product_query(text)] if _pb_v3_resolve_product_query(text) else [])
    globals()["suggested_products"] = lambda text, limit=4: _pb_v3_cosmetic_alternatives(text, limit)
    globals()["cosmetic_alternatives_for_query"] = _pb_v3_cosmetic_alternatives
    globals()["build_product_reply"] = _pb_v3_build_product_reply
    globals()["build_unavailable_reply"] = _pb_v3_build_unavailable_reply
    globals()["build_cosmetic_alternatives_direct_reply"] = _pb_v3_build_alternatives_reply
    globals()["build_not_found_reply"] = _pb_v3_build_fallback_reply
    globals()["build_welcome_reply"] = _pb_v3_build_welcome_reply
    globals()["build_unclear_image_reply"] = _pb_v3_build_unclear_image_reply
    globals()["build_image_timeout_reply"] = _pb_v3_build_image_timeout_reply
    globals()["product_from_vision"] = _pb_v3_product_from_vision
    globals()["process_image_message"] = _pb_v3_process_image_message
    globals()["build_reply"] = _pb_v3_build_reply
    globals()["_vision_prompt"] = _pb_v3_vision_prompt
    globals()["call_ai_json"] = lambda user_text: {}
    print("PRICEBOT_FINAL_SAFE_MATCH_V3 authoritative", flush=True)


_PB_V3_OLD_BUILD_REPLY = build_reply
_pb_v3_install_authoritative()


# =============================================================================
# PRICEBOT_FINAL_SAFE_MATCH_V4
# Final local matcher/vision decision layer. This block intentionally overrides
# older matching helpers without touching Meta, server settings, tokens, products,
# or the database contents.
# =============================================================================

import time as _pb_v4_time

_PB_V4_INDEX_CACHE = {"fingerprint": None, "built_at": 0.0, "entries": []}
_PB_V4_SEQUENCE_MATCHER = __import__("difflib").SequenceMatcher
_PB_V4_VISION_CACHE_VERSION = 4

_PB_V4_BRANDS = {
    "cerave": ["cerave", "cera ve", "سيرافي"],
    "la roche": ["la roche", "laroche", "la roche posay", "la roche-posay", "لاروش", "لاروش بوزيه"],
    "vichy": ["vichy", "فيشي"],
    "bioderma": ["bioderma", "بيوديرما"],
    "eucerin": ["eucerin", "يوسرين"],
    "acm": ["acm"],
    "svr": ["svr"],
    "uriage": ["uriage", "يورياج"],
    "avene": ["avene", "avène", "افين", "أفين"],
    "isispharma": ["isispharma", "isis pharma", "ايزس فارما", "إيزس فارما"],
    "the ordinary": ["the ordinary", "ordinary", "ذا اورديناري", "ذا أورديناري"],
    "arvea": ["arvea", "ارفيا", "أرفيا"],
    "alfa": ["alfa", "alpha", "الفا", "ألفا"],
}

_PB_V4_REQUEST_WORDS = {
    "متوفر", "متوفرة", "عندكم", "موجود", "موجودة", "السعر", "سعر", "بكم", "كم",
    "هل", "نبي", "ابي", "أبي", "اريد", "أريد", "لو سمحت", "من فضلك", "بالله",
    "available", "price", "do you have", "how much", "have", "need", "want", "please",
}
...
_PB_V4_TYPE_WORDS = {
    "cleanser": [
        "cleanser", "cleansing", "face wash", "facial wash", "wash", "foaming",
        "gel moussant", "gel lavant", "lavante", "lavant", "cleanser gel",
        "غسول", "منظف", "غسول وجه", "غسول بشرة", "جل منظف",
    ],
    "body_wash": ["body wash", "bodywash", "shower gel", "غسول جسم", "غسول للجسم", "للجسم"],
    "mouth_wash": [
        "mouth wash", "mouthwash", "mouth cleanser", "oral cleanser", "dental cleanser",
        "oral", "dental", "teeth", "tooth", "gum", "غسول فم", "غسول الفم",
        "فم", "الفم", "اسنان", "أسنان", "لثة", "لثه", "مضمضة",
    ],
    "shampoo": ["shampoo", "hair wash", "شامبو"],
    "lotion": ["lotion", "لوشن"],
    "moisturizer": ["moisturizer", "moisturiser", "مرطب"],
    "cream": ["cream", "كريم"],
    "serum": ["serum", "سيروم"],
    "sunscreen": ["sunscreen", "sun screen", "spf", "واقي شمس"],
    "toner": ["toner", "تونر"],
...
_PB_V4_AREA_WORDS = {
    "face": [
        "face", "facial", "visage", "for face", "face cleanser", "facial cleanser",
        "skin cleanser", "oily skin", "dry skin", "normal skin", "acne prone skin",
        "وجه", "الوجه", "بشرة", "البشرة", "غسول وجه", "غسول بشرة", "بشرة دهنية",
    ],
    "body": ["body", "body wash", "shower gel", "جسم", "للجسم", "غسول جسم"],
    "baby": ["baby", "kids", "child", "infant", "baby shark", "بيبي", "اطفال", "أطفال", "طفل"],
    "hair": ["hair", "shampoo", "scalp", "شعر", "فروة", "شامبو"],
    "mouth": [
        "mouth", "mouth wash", "mouthwash", "oral", "dental", "teeth", "tooth", "gum",
        "فم", "الفم", "اسنان", "أسنان", "لثة", "لثه", "مضمضة", "غسول فم", "غسول الفم",
    ],
}

_PB_V4_TYPE_PRIORITY = [
    "mouth_wash", "shampoo", "body_wash", "cleanser", "sunscreen", "serum",
    "lotion", "cream", "moisturizer", "toner", "oil", "gel",
    "tablets", "capsules", "syrup", "injection",
]

_PB_V4_COSMETIC_TYPES = {
    "cleanser", "lotion", "moisturizer", "cream", "serum", "sunscreen", "toner",
    "gel", "oil", "shampoo", "body_wash", "mouth_wash",
}

_PB_V4_GENERIC_WORDS = {
    "cleanser", "cleansing", "lotion", "cream", "serum", "gel", "wash", "sunscreen",
    "spf", "moisturizer", "moisturizing", "moisturising", "hydrating", "shampoo",
    "skin", "daily", "face", "facial", "body", "baby", "hair", "mouth", "oral",
    "dental", "teeth", "tooth", "gum", "normal", "dry", "oily", "for", "all",
    "cerave", "vichy", "bioderma", "eucerin", "acm", "svr", "uriage", "avene",
    "la", "roche", "isispharma", "ordinary", "arvea", "alfa",
}

_PB_V4_DESCRIPTOR_WORDS = {"hydrating", "moisturizing", "moisturising", "daily", "normal", "dry", "oily"}
_PB_V4_IMPORTANT_WORDS = {
    "sa", "baume", "effaclar", "toleriane", "foaming", "hydrating", "moisturizing",
    "moisturising", "daily", "salicylic", "acne", "rough", "bumpy", "moussant",
    "lavante", "lipikar", "cicaplast", "anthelios", "cleanance",
}
_PB_V4_UNAVAILABLE_WORDS = {
    "غير متوفر", "غير موجود", "نافذ", "نفذ", "ناقص", "لا", "no", "0",
    "out of stock", "unavailable", "not available", "ط؛ظٹط± ظ…طھظˆظپط±", "ظ†ظپط°", "ظ†ط§ظ‚طµ", "ظ„ط§",
}


def _pb_v4_log(event: str, data: dict) -> None:
    try:
        print("PRICEBOT_MATCH " + json.dumps({"event": event, **(data or {})}, ensure_ascii=False, default=str), flush=True)
    except Exception as exc:
        print(f"PRICEBOT_MATCH_LOG_ERROR {event}: {exc}", flush=True)
...
def _pb_v4_pharmacy_name() -> str:
    raw = get_config("PHARMACY_NAME", "").strip()
    if not raw or any(bad in raw for bad in ["ط", "ظ", "Ø", "Ù", "ًں"]):
        return "صيدلية بدر البشرية"
    return raw


def _pb_v4_header() -> str:
    return f"🌿 {_pb_v4_pharmacy_name()}"


def _pb_v4_basic_norm(text: str) -> str:
    s = str(text or "").strip().lower()
    if not s:
        return ""
    for src, dst in {
    ...
    s = re.sub(r"[\u064b-\u065f\u0670]", "", s)
    s = s.replace("&", " and ")
    s = re.sub(r"(?i)\bspf\s*(\d+)\b", r"spf \1", s)
    s = re.sub(r"[^\w\s\u0600-\u06FF%]+", " ", s)
    return re.sub(r"\s+", " ", s).strip()


def _pb_v4_norm(text: str, strip_request_words: bool = False) -> str:
    s = _pb_v4_basic_norm(text)
    if not s:
        return ""
    for src, dst in sorted(_PB_V4_SYNONYMS.items(), key=lambda x: len(x[0]), reverse=True):
        src_norm = _pb_v4_basic_norm(src)
        if src_norm:
            s = re.sub(rf"(?<!\w){re.escape(src_norm)}(?!\w)", dst, s)
    s = re.sub(r"\bcera\s+ve\b", "cerave", s)
    s = re.sub(r"\bla\s+roche\s+posay\b", "la roche", s)
    s = re.sub(r"\blaroche\b", "la roche", s)
    if strip_request_words:
        for phrase in sorted(_PB_V4_REQUEST_WORDS, key=len, reverse=True):
            p = _pb_v4_norm(phrase, False)
            if p:
                s = re.sub(rf"(?<!\w){re.escape(p)}(?!\w)", " ", s)
    return re.sub(r"\s+", " ", s).strip()


def _pb_v4_tokens(text: str) -> List[str]:
    return [t for t in _pb_v4_norm(text, False).split() if t]


def _pb_v4_has_phrase(norm_text: str, phrase: str) -> bool:
    p = _pb_v4_norm(phrase, False)
    if not p:
        return False
    return bool(re.search(rf"(?<!\w){re.escape(p)}(?!\w)", " " + norm_text + " "))


def _pb_v4_detect_brand(text: str) -> str:
    q = _pb_v4_norm(text, False)
    for brand, aliases in _PB_V4_BRANDS.items():
        if any(_pb_v4_has_phrase(q, alias) for alias in aliases):
            return brand
    return ""


def _pb_v4_detect_areas(text: str) -> set:
    q = _pb_v4_norm(text, False)
    out = set()
    for area, words in _PB_V4_AREA_WORDS.items():
        if any(_pb_v4_has_phrase(q, word) for word in words):
            out.add(area)
    return out


def _pb_v4_detect_types(text: str) -> set:
    q = _pb_v4_norm(text, False)
    areas = _pb_v4_detect_areas(q)
    out = set()
    has_mouth = "mouth" in areas or any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["mouth_wash"])
    has_shampoo = "hair" in areas or any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["shampoo"])
    has_body_wash = any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["body_wash"])
    has_cleanser_word = any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["cleanser"])
    ...
    if has_mouth:
        ...
        out.add("body_wash")
    elif has_cleanser_word:
        out.add("cleanser")

    has_lotion = any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["lotion"])
    has_cream = any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["cream"])
    has_moisturizer_word = any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS["moisturizer"])
    has_moisturizing_descriptor = _pb_v4_has_phrase(q, "moisturizing") or _pb_v4_has_phrase(q, "hydrating")

    if has_lotion:
        out.add("lotion")
        if has_moisturizer_word or has_moisturizing_descriptor:
            out.add("moisturizer")
    if has_cream:
        out.add("cream")
        if has_moisturizer_word or has_moisturizing_descriptor:
            out.add("moisturizer")
    if has_moisturizer_word and "cleanser" not in out and "body_wash" not in out and "shampoo" not in out and "mouth_wash" not in out:
        out.add("moisturizer")

    for typ in ["serum", "sunscreen", "toner", "oil", "tablets", "capsules", "syrup", "injection"]:
        if any(_pb_v4_has_phrase(q, w) for w in _PB_V4_TYPE_WORDS[typ]):
            out.add(typ)
    if "cleanser" not in out and "body_wash" not in out and "mouth_wash" not in out:
        if _pb_v4_has_phrase(q, "gel"):
            out.add("gel")
    return out


def _pb_v4_product_text(item: dict) -> str:
    return " ".join(str(item.get(k, "") or "") for k in [
        "name", "aliases", "active_ingredient", "brand", "company", "form", "strength", "pack", "notes"
    ])


def _pb_v4_type_words(types: set) -> set:
    words = set()
    for typ in types or set():
        words.add(_pb_v4_norm(typ, False))
        for word in _PB_V4_TYPE_WORDS.get(typ, []):
            words.update(_pb_v4_tokens(word))
    return words


def _pb_v4_area_words(areas: set) -> set:
    words = set()
    for area in areas or set():
        words.add(_pb_v4_norm(area, False))
        for word in _PB_V4_AREA_WORDS.get(area, []):
            words.update(_pb_v4_tokens(word))
    return words


def _pb_v4_strong_tokens(text: str, brand: str = "", types: Optional[set] = None, areas: Optional[set] = None) -> List[str]:
    brand_words = set()
    if brand:
        brand_words.update(_pb_v4_tokens(brand))
        for alias in _PB_V4_BRANDS.get(brand, []):
            brand_words.update(_pb_v4_tokens(alias))
    type_words = _pb_v4_type_words(types or set())
    area_words = _pb_v4_area_words(areas or set())
    out = []
    seen = set()
    for token in _pb_v4_tokens(text):
        if token in seen or token in brand_words:
            continue
        if token in type_words and token not in _PB_V4_IMPORTANT_WORDS:
            continue
        if token in area_words and token not in _PB_V4_IMPORTANT_WORDS:
            continue
        if token in _PB_V4_GENERIC_WORDS and token not in _PB_V4_IMPORTANT_WORDS:
            continue
        if len(token) < 2 and not re.search(r"\d", token):
            continue
        out.append(token)
...
def _pb_v4_parse_query(query: str, source: str = "text") -> dict:
    raw = str(query or "")
    norm = _pb_v4_norm(raw, True)
    brand = _pb_v4_detect_brand(norm)
    types = _pb_v4_detect_types(norm)
    areas = _pb_v4_detect_areas(norm)
    if "cleanser" in types and not areas:
        areas.add("face")
    if "mouth_wash" in types:
        areas.add("mouth")
    if "body_wash" in types:
        areas.add("body")
    if "shampoo" in types:
        areas.add("hair")
    size_match = re.search(r"\b\d+(?:\.\d+)?\s*(?:ml|g|gm|oz|fl oz|mg|spf)\b", norm)
    strong = _pb_v4_strong_tokens(norm, brand, types, areas)
    tokens = _pb_v4_tokens(norm)
    brand_only = bool(brand) and not types and not strong
    descriptor_only = bool(strong) and set(strong) <= _PB_V4_DESCRIPTOR_WORDS and not brand and not types
    type_only = bool(types) and not brand and not strong
    weak_category_only = brand_only or type_only or descriptor_only or (tokens and all(t in _PB_V4_GENERIC_WORDS for t in tokens))
    return {
        "raw": raw,
        "normalized": norm,
        "brand": brand,
...
def _pb_v4_split_aliases(value: str) -> List[str]:
    if not value:
        return []
    return [p.strip() for p in re.split(r"[,;|،؛\n]+", str(value)) if p and p.strip()]


def _pb_v4_alias_is_safe(alias_norm: str) -> bool:
    toks = _pb_v4_tokens(alias_norm)
    if not toks:
        return False
    if _pb_v4_detect_brand(alias_norm) and len(toks) <= 2:
        return False
    alias_types = _pb_v4_detect_types(alias_norm)
    alias_areas = _pb_v4_detect_areas(alias_norm)
    strong = _pb_v4_strong_tokens(alias_norm, _pb_v4_detect_brand(alias_norm), alias_types, alias_areas)
    if not strong and (alias_types or alias_areas or all(t in _PB_V4_GENERIC_WORDS for t in toks)):
        return False
    return True


def _pb_v4_item_terms(item: dict) -> List[Tuple[str, str, str]]:
    terms = []
    name = str(item.get("name", "") or "").strip()
    if name:
        terms.append(("name", name, _pb_v4_norm(name, False)))
    for alias in _pb_v4_split_aliases(str(item.get("aliases", "") or "")):
        norm = _pb_v4_norm(alias, False)
        if norm and _pb_v4_alias_is_safe(norm):
            terms.append(("alias", alias, norm))
    out = []
    seen = set()
    for kind, raw, norm in terms:
        if norm and norm not in seen:
...
def _pb_v4_products_index() -> List[dict]:
    products = load_products()
    fingerprint = (
        len(products),
        sum(len(str(p.get("name", "") or "")) for p in products),
    ...
    now = _pb_v4_time.time()
    if _PB_V4_INDEX_CACHE.get("fingerprint") == fingerprint and now - float(_PB_V4_INDEX_CACHE.get("built_at", 0.0)) < 60:
        return list(_PB_V4_INDEX_CACHE.get("entries") or [])
    entries = []
    for item in products:
        text = _pb_v4_product_text(item)
        name = str(item.get("name", "") or "")
        types = _pb_v4_detect_types(text)
        areas = _pb_v4_detect_areas(text)
        if "cleanser" in types and not areas:
            areas.add("face")
        if "mouth_wash" in types:
            areas.add("mouth")
        ...
        entries.append({
            "item": item,
            "name_norm": _pb_v4_norm(name, False),
            "terms": _pb_v4_item_terms(item),
            "text_norm": _pb_v4_norm(text, False),
            "brand": _pb_v4_detect_brand(text),
            "types": types,
            "areas": areas,
            "tokens": set(_pb_v4_tokens(text)),
        })
    _PB_V4_INDEX_CACHE.update({"fingerprint": fingerprint, "built_at": now, "entries": entries})
    _pb_v4_log("INDEX_BUILT", {"count": len(entries)})
    return entries


def _pb_v4_brand_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    qbrand = info.get("brand", "")
    if not qbrand:
        return True, ""
    item_brand = entry.get("brand") or _pb_v4_detect_brand(entry.get("text_norm", ""))
    if item_brand and item_brand != qbrand:
        return False, "brand_conflict"
    if not item_brand and qbrand not in entry.get("text_norm", ""):
        return False, "brand_conflict"
...
def _pb_v4_type_ok(info: dict, entry: dict, close_lotion: bool = True) -> Tuple[bool, str]:
    qtypes = set(info.get("types") or set())
    itypes = set(entry.get("types") or set())
    if not qtypes:
        return True, ""
    if not itypes:
        return False, "type_conflict"
    if "mouth_wash" in qtypes:
        return ("mouth_wash" in itypes, "" if "mouth_wash" in itypes else "type_conflict")
    if "shampoo" in qtypes:
        return ("shampoo" in itypes, "" if "shampoo" in itypes else "type_conflict")
    if "body_wash" in qtypes:
        return ("body_wash" in itypes, "" if "body_wash" in itypes else "type_conflict")
    if "cleanser" in qtypes:
        if itypes & {"serum", "lotion", "cream", "moisturizer", "sunscreen", "shampoo", "body_wash", "mouth_wash"}:
            return False, "type_conflict"
        return ("cleanser" in itypes, "" if "cleanser" in itypes else "type_conflict")
    if "serum" in qtypes:
        return ("serum" in itypes, "" if "serum" in itypes else "type_conflict")
    if "lotion" in qtypes or "moisturizer" in qtypes:
        if itypes & {"serum", "cleanser", "body_wash", "shampoo", "mouth_wash"}:
            return False, "type_conflict"
        allowed = {"lotion", "moisturizer"} if close_lotion else {"lotion"}
        return (bool(itypes & allowed), "" if itypes & allowed else "type_conflict")
    if "cream" in qtypes:
        if itypes & {"cleanser", "serum", "body_wash", "shampoo", "mouth_wash"}:
            return False, "type_conflict"
        return (bool(itypes & {"cream", "moisturizer"}), "" if itypes & {"cream", "moisturizer"} else "type_conflict")
    if "sunscreen" in qtypes:
        return ("sunscreen" in itypes, "" if "sunscreen" in itypes else "type_conflict")
    for medical in {"tablets", "capsules", "syrup", "injection"}:
        if medical in qtypes and medical not in itypes:
            return False, "type_conflict"
    return True, ""


def _pb_v4_area_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    qareas = set(info.get("areas") or set())
    iareas = set(entry.get("areas") or set())
    qtypes = set(info.get("types") or set())
    if "cleanser" in qtypes and not qareas:
    ...
    if not qareas:
        return True, ""
    if "face" in qareas:
        if iareas & {"body", "baby", "hair", "mouth"} and "face" not in iareas:
            return False, "area_conflict"
    if "mouth" in qareas:
        if "mouth" not in iareas:
            return False, "area_conflict"
    if "body" in qareas:
        if iareas and "body" not in iareas:
            return False, "area_conflict"
    if "hair" in qareas:
        if iareas and "hair" not in iareas:
            return False, "area_conflict"
    if "baby" in qareas:
        if iareas and "baby" not in iareas:
            return False, "area_conflict"
    if qareas & {"mouth", "body", "hair", "baby"} and "face" in iareas and not (qareas & iareas):
        return False, "area_conflict"
    return True, ""


def _pb_v4_candidate_ok(info: dict, entry: dict) -> Tuple[bool, str]:
    for check in (_pb_v4_brand_ok, _pb_v4_type_ok, _pb_v4_area_ok):
        ok, reason = check(info, entry)
        if not ok:
            return False, reason
    return True, ""
...
def _pb_v4_is_product_available(item: dict) -> bool:
    raw = str(item.get("available", "") or "").strip()
    if not raw:
        return True
    norm = _pb_v4_norm(raw, False)
    if norm in {"1", "true", "yes", "y", "on", "available", "متوفر", "متوفره", "متوفرة"}:
        return True
    for word in _PB_V4_UNAVAILABLE_WORDS:
        if _pb_v4_norm(word, False) and _pb_v4_norm(word, False) in norm:
            return False
    if norm in {"false", "off", "none", "لا"}:
        return False
    return True


def _pb_v4_is_cosmetic_info(info: dict) -> bool:
    return bool(set(info.get("types") or set()) & _PB_V4_COSMETIC_TYPES)


def _pb_v4_is_weak_category_only(query: str) -> bool:
    return bool(_pb_v4_parse_query(query).get("weak_category_only"))


def _pb_v4_resolve_product_query(query: str, source: str = "text") -> Optional[dict]:
    info = _pb_v4_parse_query(query, source)
    q = info["normalized"]
    rejected = []
    top = []
    if not q:
        _pb_v4_log("FINAL_DECISION", {"RAW_QUERY": query, "SOURCE": source, "FINAL_DECISION": "fallback"})
        return None
    entries = _pb_v4_products_index()
    _pb_v4_log("QUERY", {
        "RAW_QUERY": info["raw"],
        "NORMALIZED_QUERY": q,
        "SOURCE": source,
    ...
    for entry in entries:
        if q == entry["name_norm"]:
            item = entry["item"]
            _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "exact_name"})
            return item

    for entry in entries:
        ok, reason = _pb_v4_candidate_ok(info, entry)
        if not ok:
            rejected.append({"name": entry["item"].get("name", ""), "reason": reason})
            continue
        for kind, raw, norm in entry["terms"]:
            if kind == "alias" and q == norm:
                item = entry["item"]
                _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "exact_alias"})
                return item

    if info.get("weak_category_only"):
        reason = "brand_only" if info.get("brand_only") else "weak_category_only"
        _pb_v4_log("FINAL_DECISION", {
            "FINAL_DECISION": "fallback" if info.get("brand_only") or info.get("type_only") else "unavailable",
    ...
    if len(q) >= 5:
        ...
        if phrase_matches:
            phrase_matches.sort(key=lambda x: x[0], reverse=True)
            if len(phrase_matches) == 1 or phrase_matches[0][0] - phrase_matches[1][0] >= 0.04:
                item = phrase_matches[0][1]["item"]
                _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "phrase", "CANDIDATES_TOP_5": top[:5]})
                return item
            rejected.append({"name": phrase_matches[0][1]["item"].get("name", ""), "reason": "ambiguous_close_scores"})

    strong = list(info.get("strong_tokens") or [])
    if not info.get("brand") and not info.get("types") and len(strong) < 2:
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback", "REJECT_REASON": "missing_required_terms", "REJECTED": rejected[:10]})
        return None
    if info.get("types") and not strong and not info.get("brand"):
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback", "REJECT_REASON": "weak_category_only", "REJECTED": rejected[:10]})
        return None
    ...
    for entry in entries:
        ...
        missing = [t for t in strong if t not in entry_tokens]
        if missing:
            rejected.append({"name": entry["item"].get("name", ""), "reason": "missing_required_terms", "missing": missing[:4]})
            continue
        score = 0.68 + (0.08 * len(strong))
        if info.get("brand"):
            score += 0.12
        if set(info.get("types") or set()) & set(entry.get("types") or set()):
            score += 0.06
        if set(info.get("areas") or set()) & set(entry.get("areas") or set()):
            score += 0.03
        token_matches.append((min(score, 0.98), entry))
        top.append({"name": entry["item"].get("name", ""), "score": round(min(score, 0.98), 3), "stage": "tokens"})
    ...
    if token_matches:
        token_matches.sort(key=lambda x: x[0], reverse=True)
        if len(token_matches) == 1 or token_matches[0][0] - token_matches[1][0] >= 0.05:
            item = token_matches[0][1]["item"]
            _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "tokens", "CANDIDATES_TOP_5": top[:5]})
            return item
        rejected.append({"name": token_matches[0][1]["item"].get("name", ""), "reason": "ambiguous_close_scores"})
    ...
    for entry in entries:
        ...
        if not ok:
            ...
            continue
        if strong and any(t not in set(entry.get("tokens") or set()) for t in strong):
            rejected.append({"name": entry["item"].get("name", ""), "reason": "missing_required_terms"})
            continue
        best = 0.0
        for kind, raw, norm in entry["terms"]:
            best = max(best, _PB_V4_SEQUENCE_MATCHER(None, q, norm).ratio())
        if best >= 0.955:
            fuzzy.append((best, entry))
            top.append({"name": entry["item"].get("name", ""), "score": round(best, 3), "stage": "fuzzy"})
        else:
            rejected.append({"name": entry["item"].get("name", ""), "reason": "low_score", "score": round(best, 3)})
    fuzzy.sort(key=lambda x: x[0], reverse=True)
    if fuzzy and (len(fuzzy) == 1 or fuzzy[0][0] - fuzzy[1][0] >= 0.05):
        item = fuzzy[0][1]["item"]
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "matched", "FINAL_PRODUCT": item.get("name", ""), "MATCH_STAGE": "fuzzy", "CANDIDATES_TOP_5": top[:5]})
        return item

    _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "unavailable", "CANDIDATES_TOP_5": top[:5], "REJECTED": rejected[:10]})
    return None


def _pb_v4_format_price(price: str) -> str:
    price = str(price or "").strip()
    if not price:
        return ""
    return price if "د" in price else f"{price} د.ل"
...
def _pb_v4_allowed_alternative_types(info: dict) -> set:
    qtypes = set(info.get("types") or set())
    if not qtypes & _PB_V4_COSMETIC_TYPES:
        return set()
    if "cleanser" in qtypes:
        return {"cleanser"}
    if "lotion" in qtypes or "moisturizer" in qtypes:
...
def _pb_v4_same_requested_product(info: dict, entry: dict, exclude_item: Optional[dict] = None) -> bool:
    q = str(info.get("normalized") or "")
    name = str(entry.get("name_norm") or "")
    if exclude_item and _pb_v4_norm(exclude_item.get("name", ""), False) == name:
        return True
    if q and name:
        if q == name:
            return True
        if len(q) >= 8 and (q in name or name in q):
            return True
    return False


def _pb_v4_cosmetic_alternatives(text: str, limit: int = 6, exclude_item: Optional[dict] = None) -> List[dict]:
    info = _pb_v4_parse_query(text, "text")
    allowed = _pb_v4_allowed_alternative_types(info)
    if not allowed:
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "unavailable", "ALTERNATIVES": [], "REJECT_REASON": "not_cosmetic"})
        return []
    qareas = set(info.get("areas") or set())
    if "cleanser" in allowed and not qareas:
        qareas.add("face")
    ...
    for entry in entries:
        ...
        if not key or key in seen:
            continue
        if _pb_v4_same_requested_product(info, entry, exclude_item):
            continue
        if not _pb_v4_is_product_available(item):
            continue
        itypes = set(entry.get("types") or set())
        iareas = set(entry.get("areas") or set())
        if not (itypes & allowed):
            continue
        if "cleanser" in allowed:
            if itypes & {"shampoo", "body_wash", "mouth_wash"}:
                continue
            if iareas & {"baby", "body", "hair", "mouth"}:
                continue
        fake_info = dict(info)
        fake_info["types"] = allowed
        fake_info["areas"] = qareas
        ok_area, _ = _pb_v4_area_ok(fake_info, entry)
        ok_type, _ = _pb_v4_type_ok(fake_info, entry)
        if not ok_area or not ok_type:
            continue
        score = 0
        if info.get("brand") and entry.get("brand") == info.get("brand"):
            score += 6
        if qareas and iareas & qareas:
            score += 4
        score += len(itypes & allowed) * 3
        for token in info.get("strong_tokens") or []:
            if token in entry["text_norm"]:
                score += 1
        scored.append((score, item))
        seen.add(key)
    scored.sort(key=lambda x: x[0], reverse=True)
    alts = [item for score, item in scored[:limit]]
    _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "alternatives" if alts else "unavailable", "ALTERNATIVES": [a.get("name", "") for a in alts[:5]]})
    return alts


def _pb_v4_build_alternatives_reply(text: str, alts: List[dict], from_number: str = "", known_catalog_item: bool = False) -> str:
    alts = [a for a in alts if _pb_v4_is_product_available(a)]
    if from_number and alts:
        PENDING_OPTIONS[from_number] = {"items": alts[:8], "reason": "cosmetic_alternatives"}
    if not alts:
        return _pb_v4_build_unavailable_reply(text, from_number, allow_alts=False)
    best = alts[0]
    unavailable_line = (
        "المنتج موجود في قائمة الصيدلية لكنه غير متوفر حالياً."
        if known_catalog_item else
        "المنتج المطلوب غير متوفر حالياً في قائمة الصيدلية."
    )
    lines = [
        _pb_v4_header(),
    ...
    price = _pb_v4_format_price(best.get("price", ""))
    if price:
        lines.append(f"السعر: {price}")
    lines.append("")
    for i, item in enumerate(alts[:5], 1):
        suffix = _pb_v4_format_price(item.get("price", ""))
        lines.append(f"{i}) {str(item.get('name', '')).strip()}" + (f" - {suffix}" if suffix else ""))
    lines += ["", "اكتب رقم البديل المطلوب للحجز أو الاستفسار."]
    return "\n".join(lines)


def _pb_v4_build_product_reply(item: dict, original_text: str = "", from_number: str = "") -> str:
    name = str(item.get("name", "") or "").strip()
    price = _pb_v4_format_price(item.get("price", ""))
    if not _pb_v4_is_product_available(item):
        lines = [
            _pb_v4_header(),
            "",
        ...
        info = _pb_v4_parse_query(original_text or name, "text")
        if _pb_v4_is_cosmetic_info(info):
            alts = _pb_v4_cosmetic_alternatives(original_text or name, limit=6, exclude_item=item)
            if alts:
                return _pb_v4_build_alternatives_reply(original_text or name, alts, from_number, known_catalog_item=True)
        return "\n".join(lines)
    lines = [
        _pb_v4_header(),
        "",
...
def _pb_v4_build_unavailable_reply(text: str, from_number: str = "", allow_alts: bool = True) -> str:
    if allow_alts:
        alts = _pb_v4_cosmetic_alternatives(text, limit=6)
        if alts:
            return _pb_v4_build_alternatives_reply(text, alts, from_number)
    _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "unavailable", "ALTERNATIVES": []})
    return "\n".join([
        _pb_v4_header(),
        "",
        "المنتج المطلوب غير متوفر حالياً في قائمة الصيدلية.",
...
def _pb_v4_build_need_more_detail_reply(info: Optional[dict] = None) -> str:
    info = info or {}
    if info.get("brand_only"):
        detail = "يرجى تحديد نوع المنتج أو إرسال صورة أوضح، مثل CeraVe Cleanser أو CeraVe Lotion."
    elif info.get("type_only"):
        detail = "يوجد أكثر من نوع، اكتب اسم الشركة أو أرسل صورة المنتج."
    else:
        detail = "أرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."
    return f"{_pb_v4_header()}\n\n{detail}"


def _pb_v4_build_unclear_image_reply() -> str:
    return (
        f"{_pb_v4_header()}\n\n"
        "الصورة غير واضحة بما يكفي للتعرف على المنتج بدقة.\n"
        "الرجاء إرسال صورة أوضح للواجهة الأمامية للعلبة، أو كتابة اسم المنتج كما هو مكتوب عليها."
...
def _pb_v4_build_image_timeout_reply() -> str:
    return (
        f"{_pb_v4_header()}\n\n"
        "الصورة أخذت وقتاً أطول من المعتاد، الرجاء كتابة اسم المنتج أو إرسال صورة أوضح."
    )


def _pb_v4_build_fallback_reply(text: str = "") -> str:
    return f"{_pb_v4_header()}\n\nأرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."


def _pb_v4_build_welcome_reply() -> str:
    return f"{_pb_v4_header()}\n\nأرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."


def _pb_v4_is_greeting(text: str) -> bool:
    q = _pb_v4_norm(text, False)
    return q in {"السلام عليكم", "السلام", "سلام", "مرحبا", "اهلا", "اهلين", "hi", "hello", "test"} or q.startswith("السلام")


def _pb_v4_is_positive(text: str) -> bool:
    q = _pb_v4_norm(text, False)
    return q in {"نعم", "اي", "تمام", "yes", "ok", "اوكي", "حجز", "احجز"}


def _pb_v4_is_negative(text: str) -> bool:
    q = _pb_v4_norm(text, False)
    return q in {"لا", "لا شكرا", "الغاء", "إلغاء", "cancel", "no"}


def _pb_v4_build_order_success_reply(item: dict) -> str:
    return "\n".join([
        _pb_v4_header(),
        "",
        f"تم تسجيل طلب الحجز للمنتج: {str(item.get('name', '')).strip()}",
        "سيتم التواصل معك لتأكيد الطلب.",
    ])


def _pb_v4_build_options_reply(items, reason: str = "") -> str:
    items = list(items or [])
    if not items:
        return _pb_v4_build_fallback_reply()
    lines = [
        _pb_v4_header(),
        "",
    ...
    for i, item in enumerate(items[:6], 1):
        price = _pb_v4_format_price(item.get("price", ""))
        available = "متوفر" if _pb_v4_is_product_available(item) else "غير متوفر"
        lines.append(f"{i}) {str(item.get('name', '')).strip()}")
        lines.append(f"   الحالة: {available}")
        if price:
            lines.append(f"   السعر: {price}")
        lines.append("")
...
def _pb_v4_choice_from_text(text: str, items: List[dict]) -> Optional[dict]:
    q = str(text or "").strip().translate(str.maketrans("٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹", "01234567890123456789"))
    m = re.search(r"\d+", q)
    if m:
        idx = int(m.group(0)) - 1
        if 0 <= idx < len(items):
...
def _pb_v4_visible_text_lines(value) -> List[str]:
...
def _pb_v4_best_visible_name(vision: dict) -> str:
    names = vision.get("product_names")
    if isinstance(names, list):
        choices = [str(x).strip() for x in names if str(x).strip()]
    else:
        choices = []
    if str(vision.get("product_name", "") or "").strip():
        choices.insert(0, str(vision.get("product_name", "") or "").strip())
    choices.extend(_pb_v4_visible_text_lines(vision.get("visible_text", "")))
    brand = _pb_v4_detect_brand(str(vision.get("brand", "") or ""))
    best = ("", -999)
    for line in choices:
        norm = _pb_v4_norm(line, True)
        if not norm:
            continue
        if brand and _pb_v4_norm(line, False) == brand:
            continue
        if re.fullmatch(r"\d+(?:\.\d+)?\s*(ml|g|oz|fl oz|mg)", norm):
            continue
        types = _pb_v4_detect_types(norm)
        strong = _pb_v4_strong_tokens(norm, "", types, _pb_v4_detect_areas(norm))
        score = len(strong) * 3 + len(types) * 4
        if any(t in norm for t in ["for", "normal skin", "dry skin", "oily skin"]) and not types and not strong:
            score -= 5
        if 2 <= len(norm) <= 60:
            score += 1
        if score > best[1]:
...
def _pb_v4_vision_identity(data: dict) -> dict:
    data = data if isinstance(data, dict) else {}
    brand = str(data.get("brand", "") or "").strip()
    product_name = str(data.get("product_name", "") or "").strip()
    if not product_name:
        product_name = _pb_v4_best_visible_name(data)
    product_type = str(data.get("product_type", "") or data.get("type", "") or "").strip()
    target_area = str(data.get("target_area", "") or "").strip()
    size = str(data.get("size", "") or "").strip()
    base = " ".join([brand, product_name]).strip()
    queries = [
        base,
        " ".join([brand, product_name, product_type]).strip(),
        " ".join([brand, product_name, product_type, target_area]).strip(),
        " ".join([brand, product_name, product_type, size]).strip(),
    ]
    if not brand:
    ...
    return {
        ...
        "size": size,
        "internal_query": out[0] if out else "",
        "queries": out,
    }


def _pb_v4_product_from_vision(data: dict) -> Optional[dict]:
    identity = _pb_v4_vision_identity(data)
    _pb_v4_log("INTERNAL_IMAGE_QUERY", {"INTERNAL_IMAGE_QUERY": identity.get("internal_query", ""), "IMAGE_QUERIES": identity.get("queries", [])})
    for query in identity.get("queries", []):
        item = _pb_v4_resolve_product_query(query, source="image")
        if item:
            return item
    return None


def _pb_v4_process_image_message(msg: dict, from_number: str) -> str:
    start_ts = datetime.now()
    image = msg.get("image", {}) or {}
    media_id = image.get("id", "")
    caption = image.get("caption", "") or ""
    try:
        send_whatsapp_message(from_number, f"{_pb_v4_header()}\n\n🌿 جاري فحص الصورة، انتظر لحظات...")
    except Exception as exc:
        print("IMAGE ACK SEND SKIP:", exc, flush=True)
    media_bytes, mime_type, media_sha = download_whatsapp_media(media_id)
    if not media_sha and media_bytes:
        media_sha = hashlib.sha256(media_bytes).hexdigest()
    if not media_bytes:
        _pb_v4_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unclear_image", "REJECT_REASON": "media_download_failed"})
        return _pb_v4_build_unclear_image_reply()

    vision = call_gemini_vision_json(media_bytes, mime_type) if image_ai_enabled() else {}
    _pb_v4_log("AI_PARSED_JSON", {"SOURCE": "image", "AI_PARSED_JSON": vision, "vision_cache_version": _PB_V4_VISION_CACHE_VERSION})
    if not isinstance(vision, dict):
        vision = {}
    image_type = str(vision.get("image_type", "") or "").lower()
    if image_type == "timeout":
        _pb_v4_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "timeout"})
        return _pb_v4_build_image_timeout_reply()
    confidence = float(vision.get("confidence") or 0.0)
    clarity = str(vision.get("clarity", "") or "").lower()
    identity = _pb_v4_vision_identity(vision)
    useful_identity = bool(identity.get("brand") or identity.get("product_name") or identity.get("internal_query"))
    if (
        not vision
        or confidence < 0.65
        or clarity in {"bad", "unclear", "ضعيف", "غير واضح"}
        or image_type in {"unknown", "prescription_or_unclear"}
        or not useful_identity
    ):
        _pb_v4_log("FINAL_DECISION", {
            "SOURCE": "image",
            "FINAL_DECISION": "unclear_image",
            "confidence": confidence,
            "clarity": clarity,
            "image_type": image_type,
            "INTERNAL_IMAGE_QUERY": identity.get("internal_query", ""),
        })
        return _pb_v4_build_unclear_image_reply()

    item = _pb_v4_product_from_vision(vision)
    if item:
        if from_number and _pb_v4_is_product_available(item):
            LAST_PRODUCT[from_number] = item
        elif from_number:
            LAST_PRODUCT.pop(from_number, None)
        _pb_v4_log("FINAL_DECISION", {
            ...
            "FINAL_PRODUCT": item.get("name", ""),
            "seconds": (datetime.now() - start_ts).total_seconds(),
        })
        return _pb_v4_build_product_reply(item, caption or identity.get("internal_query", ""), from_number)

    internal_query = " ".join([
        identity.get("brand", ""),
        identity.get("product_name", ""),
        identity.get("product_type", ""),
        identity.get("target_area", ""),
    ]).strip()
    _pb_v4_log("FINAL_DECISION", {"SOURCE": "image", "FINAL_DECISION": "unavailable", "INTERNAL_IMAGE_QUERY": internal_query})
    return _pb_v4_build_unavailable_reply(internal_query, from_number)


def _pb_v4_build_reply(text: str, from_number: str = "") -> str:
    raw = text or ""
    info = _pb_v4_parse_query(raw, "text")
    _pb_v4_log("RAW_MESSAGE", {
        "RAW_QUERY": raw,
        "NORMALIZED_QUERY": info.get("normalized", ""),
        "SOURCE": "text",
        "DETECTED_BRAND": info.get("brand", ""),
        "DETECTED_TYPE": sorted(info.get("types") or set()),
        "DETECTED_AREA": sorted(info.get("areas") or set()),
    })
    try:
        if from_number and from_number in PENDING_OPTIONS:
            items = PENDING_OPTIONS.get(from_number, {}).get("items", [])
            if _pb_v4_is_negative(raw):
                PENDING_OPTIONS.pop(from_number, None)
                return _pb_v4_build_fallback_reply(raw)
            chosen = _pb_v4_choice_from_text(raw, items)
            if chosen:
                PENDING_OPTIONS.pop(from_number, None)
                if _pb_v4_is_product_available(chosen):
                    LAST_PRODUCT[from_number] = chosen
                else:
                    LAST_PRODUCT.pop(from_number, None)
                return _pb_v4_build_product_reply(chosen, raw, from_number)
            if _pb_v4_is_positive(raw) and len(items) > 1:
                return f"{_pb_v4_header()}\n\nاكتب رقم البديل المطلوب من القائمة، مثال: 1"
        if from_number and from_number in LAST_PRODUCT and (_pb_v4_is_positive(raw) or _pb_v4_norm(raw, False) in {"حجز", "احجز"}):
            item = LAST_PRODUCT.get(from_number)
            if not item or not _pb_v4_is_product_available(item):
                LAST_PRODUCT.pop(from_number, None)
                return f"{_pb_v4_header()}\n\nلا يمكن حجز هذا المنتج لأنه غير متوفر حالياً."
            save_order(from_number, item, raw)
            LAST_PRODUCT.pop(from_number, None)
            return _pb_v4_build_order_success_reply(item)
        if from_number and from_number in LAST_PRODUCT and _pb_v4_is_negative(raw):
            LAST_PRODUCT.pop(from_number, None)
            return _pb_v4_build_fallback_reply(raw)
    except Exception as exc:
        print("PRICEBOT_RESERVATION_SAFE_PATH_ERROR:", exc, flush=True)

    if _pb_v4_is_greeting(raw):
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback"})
        return _pb_v4_build_welcome_reply()
    if not info.get("normalized"):
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback"})
        return _pb_v4_build_fallback_reply(raw)
    if info.get("brand_only") or info.get("type_only"):
        _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback", "REJECT_REASON": "brand_only" if info.get("brand_only") else "weak_category_only"})
        return _pb_v4_build_need_more_detail_reply(info)
    try:
        if asks_medical_advice(raw):
            return build_medical_safety_reply(raw)
    except Exception:
        pass
    item = _pb_v4_resolve_product_query(raw, source="text")
    if item:
        if from_number and _pb_v4_is_product_available(item):
            LAST_PRODUCT[from_number] = item
        elif from_number:
            LAST_PRODUCT.pop(from_number, None)
        return _pb_v4_build_product_reply(item, raw, from_number)
    if info.get("brand") or info.get("types") or len(info.get("strong_tokens") or []) >= 2:
        return _pb_v4_build_unavailable_reply(raw, from_number)
    _pb_v4_log("FINAL_DECISION", {"FINAL_DECISION": "fallback"})
    return _pb_v4_build_fallback_reply(raw)


def _pb_v4_safe_ranked_products(text: str, limit: int = 8, strict: bool = True) -> List[Tuple[float, dict]]:
    item = _pb_v4_resolve_product_query(text)
    return [(1.0, item)] if item else []


def _pb_v4_query_product_matches(text: str, limit: int = 8) -> List[dict]:
    item = _pb_v4_resolve_product_query(text)
    return [item] if item else []


def _pb_v4_vision_prompt() -> str:
    return (
        "Return JSON only. Read visible product packaging text. Do not guess. "
        "Do not decide price, availability, alternatives, or whether the product exists. "
        "If text is not clear, use low confidence and clarity=bad. "
        "Extract only this schema: "
        "{\"image_type\":\"product_packaging|prescription_or_unclear|unknown\","
        "\"brand\":\"\",\"product_name\":\"\",\"product_names\":[],\"visible_text\":\"\","
        "\"product_type\":\"\",\"target_area\":\"face|body|baby|hair|mouth|unknown\","
        "\"size\":\"\",\"confidence\":0.0,\"clarity\":\"good|medium|bad\","
        "\"requires_admin_review\":false}. "
        "For CeraVe SA Cleanser use product_name='SA Cleanser', product_type='face cleanser', target_area='face'. "
        "Hydrating or moisturizing plus cleanser is still cleanser, not moisturizer."
    )


def _pb_v4_install_authoritative() -> None:
    global _PB_V4_SEQUENCE_MATCHER
    _PB_V4_SEQUENCE_MATCHER = __import__("difflib").SequenceMatcher
    globals()["pharmacy_name"] = _pb_v4_pharmacy_name
    globals()["normalize_product_query"] = lambda text, remove_request_words=True: _pb_v4_norm(text, strip_request_words=remove_request_words)
    globals()["detect_product_types"] = _pb_v4_detect_types
    globals()["product_types"] = lambda item: _pb_v4_detect_types(_pb_v4_product_text(item))
    globals()["product_identity_text"] = _pb_v4_product_text
    globals()["is_weak_only_query"] = _pb_v4_is_weak_category_only
    globals()["is_product_available"] = _pb_v4_is_product_available
    globals()["resolve_product_query"] = _pb_v4_resolve_product_query
    globals()["exact_safe_product_match"] = lambda text: _pb_v4_resolve_product_query(text)
    globals()["find_product"] = lambda text: _pb_v4_resolve_product_query(text)
    globals()["safe_ranked_products"] = _pb_v4_safe_ranked_products
    globals()["query_product_matches"] = _pb_v4_query_product_matches
    globals()["suggested_products"] = lambda text, limit=4: _pb_v4_cosmetic_alternatives(text, limit)
    globals()["cosmetic_alternatives_for_query"] = _pb_v4_cosmetic_alternatives
    globals()["build_product_reply"] = _pb_v4_build_product_reply
    globals()["build_unavailable_reply"] = _pb_v4_build_unavailable_reply
    globals()["build_cosmetic_alternatives_direct_reply"] = _pb_v4_build_alternatives_reply
    globals()["build_not_found_reply"] = _pb_v4_build_fallback_reply
    globals()["build_welcome_reply"] = _pb_v4_build_welcome_reply
    globals()["build_unclear_image_reply"] = _pb_v4_build_unclear_image_reply
    globals()["build_image_timeout_reply"] = _pb_v4_build_image_timeout_reply
    globals()["build_options_reply"] = _pb_v4_build_options_reply
    globals()["build_order_success_reply"] = _pb_v4_build_order_success_reply
    globals()["product_from_vision"] = _pb_v4_product_from_vision
    globals()["process_image_message"] = _pb_v4_process_image_message
    globals()["build_reply"] = _pb_v4_build_reply
    globals()["_vision_prompt"] = _pb_v4_vision_prompt
    globals()["call_ai_json"] = lambda user_text: {}
    print("PRICEBOT_FINAL_SAFE_MATCH_V4 authoritative", flush=True)


_pb_v4_install_authoritative()


# =============================================================================
# PRICEBOT_FINAL_SAFE_PIPELINE_V5
# Final no-silent pipeline. It builds on V4 safe matching rules and only adds
# guaranteed replies, deadlines, image-to-query routing, and send logs.
# =============================================================================

import inspect as _pb_v5_inspect
import queue as _pb_v5_queue
import threading as _pb_v5_threading
import time as _pb_v5_time

_PB_V5_TEXT_SECONDS = 5.0
_PB_V5_IMAGE_SECONDS = 20.0
_PB_V5_ALTERNATIVES_SECONDS = 3.0
_PB_V5_CONTEXT = _pb_v5_threading.local()

_PB_V5_ERROR_REPLY = (
    "🌿 صيدلية بدر البشرية\n\n"
    "عذراً، لم أتمكن من تحديد المنتج بدقة. الرجاء كتابة اسم المنتج كما هو على العلبة أو إرسال صورة أوضح."
)


def _pb_v5_current_message_id() -> str:
    return str(getattr(_PB_V5_CONTEXT, "message_id", "") or "")


def _pb_v5_set_context(message_id: str = "", source: str = "") -> None:
    if message_id:
        _PB_V5_CONTEXT.message_id = str(message_id)
    if source:
        _PB_V5_CONTEXT.source = str(source)


def _pb_v5_log(event: str, data: Optional[dict] = None) -> None:
    payload = dict(data or {})
    payload.setdefault("event", event)
    mid = payload.get("MESSAGE_ID") or _pb_v5_current_message_id()
    if mid:
        payload["MESSAGE_ID"] = mid
    try:
        print("PRICEBOT_MATCH " + json.dumps(payload, ensure_ascii=False, default=str), flush=True)
    except Exception as exc:
        print(f"PRICEBOT_V5_LOG_ERROR {event}: {exc}", flush=True)


def _pb_v5_message_id(msg: Optional[dict] = None, from_number: str = "", source: str = "", raw: str = "") -> str:
    msg = msg or {}
    explicit = str(msg.get("id", "") or msg.get("message_id", "") or "").strip()
    if explicit:
        return explicit[-18:]
    seed = "|".join([str(from_number or ""), str(source or ""), str(raw or ""), str(_pb_v5_time.time_ns())])
    return hashlib.sha1(seed.encode("utf-8", errors="ignore")).hexdigest()[:12]


def _pb_v5_deadline(seconds: float) -> float:
    return _pb_v5_time.monotonic() + max(0.1, float(seconds or 0.1))


def _pb_v5_remaining(deadline: Optional[float]) -> float:
    if not deadline:
        return 999.0
    return max(0.0, float(deadline) - _pb_v5_time.monotonic())


def _pb_v5_expired(deadline: Optional[float], reserve: float = 0.0) -> bool:
    return bool(deadline and _pb_v5_remaining(deadline) <= reserve)


def _pb_v5_call_with_timeout(label: str, fn, timeout_seconds: float, fallback=None, message_id: str = ""):
    timeout_seconds = max(0.01, float(timeout_seconds or 0.01))
    result_q = _pb_v5_queue.Queue(maxsize=1)

    def _target():
        try:
            result_q.put(("ok", fn()), block=False)
        except Exception as exc:
            try:
    ...
    worker = _pb_v5_threading.Thread(target=_target, name=f"pricebot-v5-{label}", daemon=True)
    worker.start()
    try:
        status, value = result_q.get(timeout=timeout_seconds)
    except _pb_v5_queue.Empty:
        _pb_v5_log("TIMEOUT", {"MESSAGE_ID": message_id, "STAGE": label, "timeout_seconds": timeout_seconds})
        return fallback, "timeout"
    if status == "error":
        _pb_v5_log("STAGE_ERROR", {"MESSAGE_ID": message_id, "STAGE": label, "error": str(value)})
        return fallback, "error"
    return value, "ok"


def _pb_v5_safe_header() -> str:
    try:
        return _pb_v4_header()
    except Exception:
        return "🌿 صيدلية بدر البشرية"


def _pb_v5_safe_error_reply() -> str:
    return _PB_V5_ERROR_REPLY


def _pb_v5_unavailable_reply() -> str:
    return "\n".join([
        _pb_v5_safe_header(),
        "",
        "المنتج المطلوب غير متوفر حالياً في قائمة الصيدلية.",
...
def _pb_v5_known_unavailable_reply() -> str:
    return "\n".join([
        _pb_v5_safe_header(),
        "",
        "المنتج موجود في قائمة الصيدلية لكنه غير متوفر حالياً.",
...
def _pb_v5_timeout_reply(source: str = "text") -> str:
    if source == "image":
        return "\n".join([
            _pb_v5_safe_header(),
            "",
    ...
    return "\n".join([
        _pb_v5_safe_header(),
        "",
        "لم أتمكن من تحديد المنتج بدقة، الرجاء كتابة اسم المنتج كما هو على العلبة أو إرسال صورة أوضح.",
    ])


def _pb_v5_need_more_detail_reply(info: dict) -> str:
    try:
        return _pb_v4_build_need_more_detail_reply(info)
    except Exception:
        if info.get("brand_only"):
            ...
            detail = "أرسل اسم المنتج أو صورته للبحث عن السعر والتوفر."
        return f"{_pb_v5_safe_header()}\n\n{detail}"


def _pb_v5_parse_query(query: str, source: str = "text") -> dict:
    try:
        return _pb_v4_parse_query(query, source)
    except Exception as exc:
        _pb_v5_log("STAGE_ERROR", {"STAGE": "parse_query", "error": str(exc), "SOURCE": source})
        return {
            "raw": query or "",
            "normalized": _pb_v4_norm(query or "", True) if "_pb_v4_norm" in globals() else str(query or "").strip().lower(),
            "brand": "",
            "types": set(),
            "areas": set(),
            "strong_tokens": [],
            "brand_only": False,
            "type_only": False,
            "weak_category_only": False,
            "source": source,
        }


def _pb_v5_safe_alternatives(query: str, info: dict, from_number: str, deadline: float, message_id: str, exclude_item: Optional[dict] = None) -> List[dict]:
    if _pb_v5_expired(deadline, 0.2):
        _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "timeout_fallback", "STAGE": "alternatives_precheck"})
        return []
    try:
        is_cosmetic = _pb_v4_is_cosmetic_info(info)
    except Exception:
        is_cosmetic = bool(set(info.get("types") or set()) & _PB_V4_COSMETIC_TYPES)
    if not is_cosmetic:
        return []
    alt_timeout = min(_PB_V5_ALTERNATIVES_SECONDS, max(0.05, _pb_v5_remaining(deadline) - 0.15))
    if alt_timeout <= 0.05:
        return []

    def _find_alts():
        return _pb_v4_cosmetic_alternatives(query, limit=6, exclude_item=exclude_item)

    alts, status = _pb_v5_call_with_timeout("alternatives", _find_alts, alt_timeout, [], message_id)
    if status != "ok" or not isinstance(alts, list):
        return []
    safe = []
    for item in alts:
        try:
            if item and _pb_v4_is_product_available(item):
                safe.append(item)
        except Exception:
            continue
    return safe[:6]
...
def _pb_v5_direct_resolve(resolve_query: str, source: str, message_id: str) -> Tuple[Optional[dict], str]:
    try:
        _pb_v5_log("MATCH_ATTEMPT", {
            "MESSAGE_ID": message_id,
            "STAGE": "resolve_product_query_direct",
            "RAW_QUERY": resolve_query,
            "SOURCE": source,
        })
        item = _pb_v4_resolve_product_query(resolve_query, source=source)
        _pb_v5_log("MATCH_RESULT", {
            "MESSAGE_ID": message_id,
            "STAGE": "resolve_product_query_direct",
            "FOUND": bool(item),
            "FINAL_PRODUCT": item.get("name", "") if item else "",
        })
        return item, "ok"
    except Exception as exc:
        _pb_v5_log("STAGE_ERROR", {
            "MESSAGE_ID": message_id,
            "STAGE": "resolve_product_query_direct",
            "error": str(exc),
        })
...
def _pb_v5_alternatives_reply(query: str, alts: List[dict], from_number: str, known_catalog_item: bool = False) -> str:
    try:
        return _pb_v4_build_alternatives_reply(query, alts, from_number, known_catalog_item=known_catalog_item)
    except TypeError:
        return _pb_v4_build_alternatives_reply(query, alts, from_number)
    except Exception as exc:
        _pb_v5_log("STAGE_ERROR", {"STAGE": "build_alternatives_reply", "error": str(exc)})
        return _pb_v5_known_unavailable_reply() if known_catalog_item else _pb_v5_unavailable_reply()


def _pb_v5_product_reply(item: dict, query: str, from_number: str, message_id: str) -> str:
    try:
        if _pb_v4_is_product_available(item):
            if from_number:
                LAST_PRODUCT[from_number] = item
            reply = _pb_v4_build_product_reply(item, query, from_number)
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "FINAL_DECISION": "matched",
                "FINAL_PRODUCT": item.get("name", ""),
            })
            return reply or _pb_v5_safe_error_reply()
        if from_number:
            LAST_PRODUCT.pop(from_number, None)
        _pb_v5_log("PRODUCT_UNAVAILABLE_IN_CATALOG", {"MESSAGE_ID": message_id, "FINAL_PRODUCT": item.get("name", "")})
        return ""
    except Exception as exc:
        _pb_v5_log("STAGE_ERROR", {"MESSAGE_ID": message_id, "STAGE": "build_product_reply", "error": str(exc)})
        return _pb_v5_safe_error_reply()


def handle_product_query(query: str, source: str = "text", from_number: str = "", message_id: str = "", deadline: Optional[float] = None) -> str:
    source = source or "text"
    message_id = message_id or _pb_v5_current_message_id() or _pb_v5_message_id(from_number=from_number, source=source, raw=query)
    _pb_v5_set_context(message_id, source)
    deadline = deadline or _pb_v5_deadline(_PB_V5_IMAGE_SECONDS if source == "image" else _PB_V5_TEXT_SECONDS)
    raw = str(query or "").strip()
    try:
        info = _pb_v5_parse_query(raw, source)
        _pb_v5_log("HANDLE_PRODUCT_QUERY", {
            "MESSAGE_ID": message_id,
            "RAW_QUERY": raw,
            "NORMALIZED_QUERY": info.get("normalized", ""),
            "SOURCE": source,
        ...
        if not raw or not info.get("normalized"):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback"})
            return _pb_v4_build_fallback_reply(raw) if "_pb_v4_build_fallback_reply" in globals() else _pb_v5_safe_error_reply()

        if _pb_v5_expired(deadline, 0.05):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "timeout_fallback", "SOURCE": source})
            return _pb_v5_timeout_reply(source)

        if info.get("brand_only") or info.get("type_only"):
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "FINAL_DECISION": "fallback",
                "REJECT_REASON": "brand_only" if info.get("brand_only") else "weak_category_only",
            })
            return _pb_v5_need_more_detail_reply(info)

        resolve_queries = [raw]
        if source == "image":
            for candidate in list(getattr(_PB_V5_CONTEXT, "image_query_variants", []) or []):
                candidate = str(candidate or "").strip()
                if candidate and candidate not in resolve_queries:
                    resolve_queries.append(candidate)
        ...
        for resolve_query in resolve_queries:
            if _pb_v5_expired(deadline, 0.05):
                _pb_v5_log("FINAL_DECISION", {
                    ...
                    "STAGE": "resolve_precheck",
                    "remaining_seconds": round(_pb_v5_remaining(deadline), 3),
                })
                return _pb_v5_timeout_reply(source)
            candidate_item, status = _pb_v5_direct_resolve(resolve_query, source, message_id)
            if status == "error":
                # A resolver error is a real internal failure, not a product miss.
                _pb_v5_log("FINAL_DECISION", {
                    "MESSAGE_ID": message_id,
                    "FINAL_DECISION": "error_fallback",
                    "STAGE": "resolve_product_query",
                })
                return _pb_v5_safe_error_reply()
            if candidate_item:
                item = candidate_item
                matched_query = resolve_query
                break

        if item:
            product_reply = _pb_v5_product_reply(item, matched_query, from_number, message_id)
            if product_reply:
                return product_reply
            # Product exists but is unavailable. Try safe cosmetic alternatives only within the budget.
            alts = _pb_v5_safe_alternatives(raw, info, from_number, deadline, message_id, exclude_item=item)
            if alts:
                _pb_v5_log("FINAL_DECISION", {
                    "MESSAGE_ID": message_id,
                    "FINAL_DECISION": "alternatives",
                    "FINAL_PRODUCT": item.get("name", ""),
                    "ALTERNATIVES": [a.get("name", "") for a in alts[:5]],
                })
                return _pb_v5_alternatives_reply(raw, alts, from_number, known_catalog_item=True)
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "FINAL_DECISION": "unavailable",
                "FINAL_PRODUCT": item.get("name", ""),
                "known_catalog_item": True,
            })
            return _pb_v5_known_unavailable_reply()

        if _pb_v5_expired(deadline, 0.05):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "timeout_fallback", "STAGE": "post_resolve"})
            return _pb_v5_timeout_reply(source)

        if not info.get("brand") and not info.get("types") and len(info.get("strong_tokens") or []) < 2:
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "FINAL_DECISION": "fallback",
        ...
        alts = _pb_v5_safe_alternatives(raw, info, from_number, deadline, message_id)
        if alts:
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "FINAL_DECISION": "alternatives",
            ...
            return _pb_v5_alternatives_reply(raw, alts, from_number, known_catalog_item=False)

        _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "unavailable"})
        return _pb_v5_unavailable_reply()
...
def _pb_v5_vision_identity(data: dict) -> dict:
    data = data if isinstance(data, dict) else {}
    brand = str(data.get("brand", "") or "").strip()
    product_name = str(data.get("product_name", "") or "").strip()
    if not product_name and isinstance(data.get("product_names"), list):
        product_name = " ".join(str(x).strip() for x in data.get("product_names") if str(x).strip()).strip()
    if not product_name:
        try:
            product_name = _pb_v4_best_visible_name(data)
        except Exception:
    ...
    size = str(data.get("size", "") or "").strip()
    internal_query = " ".join([brand, product_name, product_type, target_area, size]).strip()
    base_query = " ".join([brand, product_name]).strip()
    typed_query = " ".join([brand, product_name, product_type]).strip()
    typed_size_query = " ".join([brand, product_name, product_type, size]).strip()
    variants = []
    seen = set()
    for candidate in [internal_query, base_query, typed_query, typed_size_query, product_name if not brand else ""]:
        key = _pb_v4_norm(candidate, True) if candidate else ""
        if key and key not in seen:
...
def _pb_v5_product_from_vision(data: dict) -> Optional[dict]:
    identity = _pb_v5_vision_identity(data)
    internal_query = identity.get("internal_query", "")
    _pb_v5_log("INTERNAL_IMAGE_QUERY", {"INTERNAL_IMAGE_QUERY": internal_query})
    if not internal_query:
        return None
    item, status = _pb_v5_call_with_timeout(
        "product_from_vision_resolve",
        lambda: _pb_v4_resolve_product_query(internal_query, source="image"),
        5.0,
        None,
        _pb_v5_current_message_id(),
    )
    return item if status == "ok" else None


def _pb_v5_unclear_image_reply() -> str:
    try:
        return _pb_v4_build_unclear_image_reply()
    except Exception:
        return (
            f"{_pb_v5_safe_header()}\n\n"
            "الصورة غير واضحة بما يكفي للتعرف على المنتج بدقة.\n"
            "الرجاء إرسال صورة أوضح للواجهة الأمامية للعلبة، أو كتابة اسم المنتج كما هو مكتوب عليها."
        )


def _pb_v5_process_image_message(msg: dict, from_number: str) -> str:
    message_id = _pb_v5_message_id(msg, from_number, "image", "")
    _pb_v5_set_context(message_id, "image")
    deadline = _pb_v5_deadline(_PB_V5_IMAGE_SECONDS)
    image = (msg or {}).get("image", {}) or {}
    media_id = image.get("id", "")
    try:
        send_whatsapp_message(from_number, f"{_pb_v5_safe_header()}\n\n🌿 جاري فحص الصورة، انتظر لحظات...")
    except Exception as exc:
        _pb_v5_log("SEND_ERROR", {"MESSAGE_ID": message_id, "STAGE": "image_ack", "error": str(exc)})
    try:
        media_bytes, mime_type, media_sha = download_whatsapp_media(media_id)
        if not media_bytes:
            _pb_v5_log("FINAL_DECISION", {
            ...
            return _pb_v5_unclear_image_reply()
        if _pb_v5_expired(deadline, 0.2):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "SOURCE": "image", "FINAL_DECISION": "timeout_fallback", "STAGE": "media_download"})
            return _pb_v5_timeout_reply("image")

        vision_timeout = min(18.0, max(0.1, _pb_v5_remaining(deadline) - 0.4))
        vision, status = _pb_v5_call_with_timeout(
            "vision_ai",
            lambda: call_gemini_vision_json(media_bytes, mime_type) if image_ai_enabled() else {},
            vision_timeout,
            {},
            message_id,
        )
        if status == "timeout":
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "SOURCE": "image", "FINAL_DECISION": "timeout_fallback", "STAGE": "vision_ai"})
            return _pb_v5_timeout_reply("image")
        if not isinstance(vision, dict):
            vision = {}
        _pb_v5_log("AI_PARSED_JSON", {
            "MESSAGE_ID": message_id,
            "SOURCE": "image",
            "AI_PARSED_JSON": vision,
            "vision_cache_version": _PB_V4_VISION_CACHE_VERSION if "_PB_V4_VISION_CACHE_VERSION" in globals() else 5,
        })

        image_type = str(vision.get("image_type", "") or "").lower()
        if image_type == "timeout":
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "SOURCE": "image", "FINAL_DECISION": "timeout_fallback"})
            return _pb_v5_timeout_reply("image")
        try:
            confidence = float(vision.get("confidence") or 0.0)
        except Exception:
            confidence = 0.0
        clarity = str(vision.get("clarity", "") or "").lower()
        identity = _pb_v5_vision_identity(vision)
        internal_query = identity.get("internal_query", "")
        _PB_V5_CONTEXT.image_query_variants = identity.get("queries", []) or [internal_query]
        _pb_v5_log("INTERNAL_IMAGE_QUERY", {
            "MESSAGE_ID": message_id,
            "SOURCE": "image",
            "INTERNAL_IMAGE_QUERY": internal_query,
            "IMAGE_QUERY_VARIANTS": identity.get("queries", []),
        })
        ...
        if _pb_v5_expired(deadline, 0.1):
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
                "SOURCE": "image",
                "FINAL_DECISION": "unavailable",
                "STAGE": "deadline_after_ai_parsed",
                "INTERNAL_IMAGE_QUERY": internal_query,
            })
            return _pb_v5_unavailable_reply()
        return handle_product_query(internal_query, source="image", from_number=from_number, message_id=message_id, deadline=deadline)
...
def _pb_v5_is_positive(text: str) -> bool:
    try:
        return _pb_v4_is_positive(text)
    except Exception:
        q = _pb_v4_norm(text or "", False)
        return q in {"نعم", "اي", "تمام", "yes", "ok", "اوكي", "حجز", "احجز"}


def _pb_v5_is_negative(text: str) -> bool:
    try:
        return _pb_v4_is_negative(text)
    except Exception:
        q = _pb_v4_norm(text or "", False)
        return q in {"لا", "لا شكرا", "الغاء", "cancel", "no"}


def _pb_v5_build_reply(text: str, from_number: str = "") -> str:
    raw = text or ""
    message_id = _pb_v5_message_id(from_number=from_number, source="text", raw=raw)
    _pb_v5_set_context(message_id, "text")
    deadline = _pb_v5_deadline(_PB_V5_TEXT_SECONDS)
    try:
        info = _pb_v5_parse_query(raw, "text")
        _pb_v5_log("RAW_MESSAGE", {
            "MESSAGE_ID": message_id,
            "RAW_QUERY": raw,
            "NORMALIZED_QUERY": info.get("normalized", ""),
            "SOURCE": "text",
        ...
        try:
            if from_number and from_number in PENDING_OPTIONS:
                items = PENDING_OPTIONS.get(from_number, {}).get("items", [])
                if _pb_v5_is_negative(raw):
                    PENDING_OPTIONS.pop(from_number, None)
                    _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "cancel_options"})
                    return _pb_v4_build_fallback_reply(raw)
                chosen = _pb_v4_choice_from_text(raw, items) if "_pb_v4_choice_from_text" in globals() else None
                if chosen:
                    PENDING_OPTIONS.pop(from_number, None)
                    return handle_product_query(str(chosen.get("name", "") or ""), "text", from_number, message_id, deadline)
                if _pb_v5_is_positive(raw) and len(items) > 1:
                    _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "option_number_required"})
                    return f"{_pb_v5_safe_header()}\n\nاكتب رقم البديل المطلوب من القائمة، مثال: 1"

            if from_number and from_number in LAST_PRODUCT and (_pb_v5_is_positive(raw) or _pb_v4_norm(raw, False) in {"حجز", "احجز"}):
                item = LAST_PRODUCT.get(from_number)
                if not item or not _pb_v4_is_product_available(item):
                    LAST_PRODUCT.pop(from_number, None)
                    _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "reject_unavailable_reservation"})
                    return f"{_pb_v5_safe_header()}\n\nلا يمكن حجز هذا المنتج لأنه غير متوفر حالياً."
                save_order(from_number, item, raw)
                LAST_PRODUCT.pop(from_number, None)
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "matched", "STAGE": "reservation_confirmed", "FINAL_PRODUCT": item.get("name", "")})
                return _pb_v4_build_order_success_reply(item) if "_pb_v4_build_order_success_reply" in globals() else build_order_success_reply(item)

            if from_number and from_number in LAST_PRODUCT and _pb_v5_is_negative(raw):
                LAST_PRODUCT.pop(from_number, None)
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "cancel_last_product"})
                return _pb_v4_build_fallback_reply(raw)
        except Exception as exc:
            _pb_v5_log("STAGE_ERROR", {"MESSAGE_ID": message_id, "STAGE": "reservation_or_options", "error": str(exc)})

        try:
            if _pb_v4_is_greeting(raw):
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "greeting"})
                return _pb_v4_build_welcome_reply()
        except Exception:
            pass

        if not info.get("normalized"):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "empty_text"})
            return _pb_v4_build_fallback_reply(raw)
        if info.get("brand_only") or info.get("type_only"):
            _pb_v5_log("FINAL_DECISION", {
                "MESSAGE_ID": message_id,
        ...
        try:
            if asks_medical_advice(raw):
                reply = build_medical_safety_reply(raw)
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "fallback", "STAGE": "medical_safety"})
                return reply or _pb_v5_safe_error_reply()
        except Exception:
            pass
        return handle_product_query(raw, source="text", from_number=from_number, message_id=message_id, deadline=deadline)
    except Exception as exc:
        _pb_v5_log("FINAL_DECISION", {
            "MESSAGE_ID": message_id,
            "SOURCE": "text",
...
def _pb_v5_wrap_send(fn):
    if not callable(fn) or getattr(fn, "_pb_v5_wrapped_send", False):
        return fn

    if _pb_v5_inspect.iscoroutinefunction(fn):
        async def _async_send_wrapper(*args, **kwargs):
            if getattr(_PB_V5_CONTEXT, "in_send_log", False):
                return await fn(*args, **kwargs)
            message_id = _pb_v5_current_message_id()
            to_number = str(args[0] if args else kwargs.get("to_number", "") or kwargs.get("to", ""))
            body = str(args[1] if len(args) > 1 else kwargs.get("message", "") or kwargs.get("body", ""))
            _pb_v5_log("SEND_ATTEMPT", {"MESSAGE_ID": message_id, "to": to_number, "chars": len(body)})
            _PB_V5_CONTEXT.in_send_log = True
            try:
                result = await fn(*args, **kwargs)
                _pb_v5_log("SEND_OK" if result else "SEND_ERROR", {"MESSAGE_ID": message_id, "to": to_number, "result": bool(result)})
                return result
            except Exception as exc:
                _pb_v5_log("SEND_ERROR", {"MESSAGE_ID": message_id, "to": to_number, "error": str(exc)})
                raise
            finally:
                _PB_V5_CONTEXT.in_send_log = False
        _async_send_wrapper._pb_v5_wrapped_send = True
        return _async_send_wrapper
    ...
    def _send_wrapper(*args, **kwargs):
        ...
        try:
            ...
            _PB_V5_CONTEXT.in_send_log = False
    _send_wrapper._pb_v5_wrapped_send = True
    return _send_wrapper


def _pb_v5_install_authoritative() -> None:
    globals()["handle_product_query"] = handle_product_query
    globals()["product_from_vision"] = _pb_v5_product_from_vision
    globals()["process_image_message"] = _pb_v5_process_image_message
    globals()["build_reply"] = _pb_v5_build_reply
    globals()["send_whatsapp_message"] = _pb_v5_wrap_send(globals().get("send_whatsapp_message"))
    globals()["call_ai_json"] = lambda user_text: {}
    print("PRICEBOT_FINAL_SAFE_PIPELINE_V5 authoritative", flush=True)


_pb_v5_install_authoritative()
...
# Keep the final safe matcher authoritative after legacy wrapper hotfixes.
try:
    _pb_v3_install_authoritative()
except Exception as _final_e:
    print("PRICEBOT_FINAL_SAFE_MATCH_V3 restore error:", _final_e)

try:
    _pb_v4_install_authoritative()
except Exception as _final_v4_e:
    print("PRICEBOT_FINAL_SAFE_MATCH_V4 restore error:", _final_v4_e)

try:
    _pb_v5_install_authoritative()
except Exception as _final_v5_e:
    print("PRICEBOT_FINAL_SAFE_PIPELINE_V5 restore error:", _final_v5_e)
