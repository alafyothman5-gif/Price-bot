...
def _entry_aliases(item: dict) -> Set[str]:
    aliases: Set[str] = set()
    for field_name in ["aliases", "image_ocr_keywords", "ocr_keywords", "keywords"]:
...
def _build_product_index(products: List[dict]) -> ProductIndex:
    ...
    return ProductIndex(entries, normalized_name_map, alias_map, brand_index, type_index, area_index)
...
def safe_match(query: str) -> Tuple[str, Optional[dict]]:
    ...
    # Critical fast path: generic brand/category queries must not touch the product table.
...
def handle_text_query(phone: str, text: str, user_state: dict) -> str:
    ...
    if q_norm in GREETINGS_NORM or q_clean in GREETINGS_NORM:
        return with_header("مرحباً بك. أرسل اسم المنتج أو صورته للبحث عن السعر والتوفر.")
    ...
    if q_norm in YES_WORDS_NORM:
        ...
        if item and is_available(item.get("available", "متوفر")):
            ...
            database.clear_user_state(phone)
            return with_header(f"✅ تم تسجيل طلب الحجز للمنتج:\n{item.get('name', '')}\nسيتم التواصل معك قريباً للتأكيد.")
        database.clear_user_state(phone)
        return with_header("لا يوجد منتج متاح للحجز حالياً. الرجاء البحث عن منتج متوفر أولاً.")

    if q_norm in NO_WORDS_NORM:
        database.clear_user_state(phone)
        return with_header("تم الإلغاء. يمكنك البحث عن منتج آخر.")
    ...
    if status == "BRAND_ONLY":
        return build_brand_only_reply()
    if status == "CATEGORY_ONLY":
        return build_category_only_reply()
    if status == "AMBIGUOUS":
