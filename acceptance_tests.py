...
def text_tests():
    ...
    assert_contains(test_reply, "غير موجود", "unknown code-like query is unavailable, not random match")
    assert database.get_query_misses(10), "query miss logging records fallback/unavailable decisions"
    ...
    assert_contains(reply("t6_fuzzy_panadol", "متوفر بنادووول"), "Panadol 500mg Tablets", "fuzzy arabic typo panadol")
    assert_contains(reply("t6_fuzzy_cerave", "Cearave moisturising lotion"), "CeraVe Moisturising Lotion Baume", "fuzzy english typo cerave")
    database.add_synonym("حبة ١٢٣", "123")
    assert_contains(reply("t6_dynamic_syn", "متوفر حبة ١٢٣"), "✅ المنتج: 123", "dynamic synonym without restart")
    ...
    assert_contains(med_missing, "غير موجود", "missing medicine unavailable")
...
async def image_tests():
    ...
    assert_contains(cleanser_reply, "غير موجود", "missing cleanser unavailable")
...
def booking_tests():
    ...
    assert_contains(reply("b3", "CeraVe Renewing SA Cleanser"), "غير موجود", "unavailable before yes")
...
def performance_tests_4991():
    ...
    timed_reply("test123", "p2", "test123", 6.0)
    ...
    assert_contains(timed_reply("Cerave moisturising lotion Baume", "p6", "Cerave moisturising lotion Baume", 4.0), "CeraVe Moisturising Lotion Baume", "perf cerave lotion")
    assert_contains(timed_reply("متوفر عندكم بانادول", "p7", "متوفر عندكم بانادول", 4.0), "Panadol 500mg Tablets", "perf panadol")
    assert_contains(timed_reply("متوفر عندكم 123", "p7_code", "متوفر عندكم 123", 4.0), "123", "perf numeric product")
    assert_contains(timed_reply("متوفر كونجيستال", "p7_congestal", "متوفر كونجيستال", 4.0), "Congestal 20tab", "perf arabic congestal")
    assert_contains(timed_reply("متوفر لاروش سيكا", "p7_cica", "متوفر لاروش سيكا", 4.0), "Cicaplast b5+", "perf arabic cica")
    assert_contains(timed_reply("CeraVe Renewing SA Cleanser", "p8", "CeraVe Renewing SA Cleanser", 5.0), "غير موجود", "perf missing cleanser")
