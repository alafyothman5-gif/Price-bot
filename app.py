...
def empty_memory() -> dict:
    return {
        "version": 1,
        "query_cache": {},        # normalized query -> product/options/ai result
        "product_alias_memory": {},
        "image_cache": {},        # image sha256 -> product/review result
        "admin_corrections": {},  # normalized user phrase -> product name
        "ai_usage": {},           # YYYY-MM-DD -> counters
    }
...
def load_memory() -> dict:
    ...
    try:
        ...
        if isinstance(data, dict):
            base.update(data)
            for k in ["query_cache", "product_alias_memory", "image_cache", "admin_corrections", "ai_usage"]:
                if not isinstance(base.get(k), dict):
                    base[k] = {}
            return base
    except Exception as exc:
        print(f"MEMORY LOAD WARNING: {exc}", flush=True)
    return empty_memory()


def save_memory(data: dict) -> None:
    MEMORY_FILE.parent.mkdir(parents=True, exist_ok=True)
    fd, tmp_name = tempfile.mkstemp(prefix="memory_", suffix=".json", dir=str(MEMORY_FILE.parent))
    os.close(fd)
...
def memory_get_admin_correction(text: str) -> Optional[dict]:
    ...
    product_name = mem.get("admin_corrections", {}).get(key) or mem.get("product_alias_memory", {}).get(key)
    if product_name:
...
def memory_get_query_response(text: str) -> dict:
    mem = load_memory()
    key = memory_query_key(text)
    entry = mem.get("query_cache", {}).get(key)
    if not isinstance(entry, dict):
        return {}
    entry["hits"] = int(entry.get("hits", 0) or 0) + 1
    entry["last_hit"] = now_str()
    mem["query_cache"][key] = entry
    save_memory(mem)
    return entry
...
def memory_remember_product_query(text: str, item: dict, source: str = "local") -> None:
    ...
    if not key or not product_name:
        return
    mem = load_memory()
    mem.setdefault("query_cache", {})[key] = {
        "type": "product",
    ...
    mem.setdefault("product_alias_memory", {})[key] = product_name
    save_memory(mem)
...
def memory_lookup_image(sha: str) -> dict:
    if not sha:
        return {}
    mem = load_memory()
    entry = mem.get("image_cache", {}).get(sha)
    if isinstance(entry, dict):
        entry["hits"] = int(entry.get("hits", 0) or 0) + 1
        entry["last_hit"] = now_str()
        mem["image_cache"][sha] = entry
        save_memory(mem)
        return entry
    return {}
...
def memory_remember_image_product(sha: str, item: dict, source: str = "gemini_vision") -> None:
    if not sha or not item:
        return
    mem = load_memory()
    mem.setdefault("image_cache", {})[sha] = {
        "type": "product",
...
def memory_record_ai_usage(kind: str, provider: str = "unknown") -> None:
    ...
    usage = mem.setdefault("ai_usage", {}).setdefault(day, {})
    key = f"{kind}:{provider or 'unknown'}"
    usage[key] = int(usage.get(key, 0) or 0) + 1
    mem["ai_usage"][day] = usage
    save_memory(mem)
...
def memory_stats() -> dict:
    ...
    return {
        "query_cache": len(mem.get("query_cache", {})),
        "product_alias_memory": len(mem.get("product_alias_memory", {})),
        "image_cache": len(mem.get("image_cache", {})),
        "admin_corrections": len(mem.get("admin_corrections", {})),
        "ai_usage_today": mem.get("ai_usage", {}).get(today, {}),
    }
...
def _row_get(row: dict, *names: str, default: str = "") -> str:
    ...
    # fallback by normalized header names
...
def load_products() -> List[dict]:
    ...
    with PRODUCTS_FILE.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            name = _row_get(row, "name", "product", "product_name", "اسم", "الاسم", "اسم المنتج")
            if not name:
                continue
            item = {
                "name": name,
                "aliases": _row_get(row, "aliases", "alias", "أسماء بديلة", "اسماء بديلة", "بدائل"),
                "active_ingredient": _row_get(row, "active_ingredient", "ingredient", "generic", "المادة الفعالة", "ماده فعاله", "المادة", "ماده"),
                "brand": _row_get(row, "brand", "trade_name", "الشركة", "الماركة", "براند", "اسم تجاري"),
                "company": _row_get(row, "company", "manufacturer", "origin", "country", "الشركة", "الشركه", "المنشأ", "بلد", "البلد"),
                "form": _row_get(row, "form", "dosage_form", "shape", "الشكل", "الشكل الدوائي", "نوع", "النوع"),
                "strength": _row_get(row, "strength", "concentration", "dose", "التركيز", "جرعة", "عيار"),
                "pack": _row_get(row, "pack", "package", "pack_size", "العبوة", "عبوة", "التعبئة"),
                "price": _row_get(row, "price", "سعر", "السعر"),
                "available": _row_get(row, "available", "availability", "stock", "توفر", "التوفر", "الحالة", default="متوفر") or "متوفر",
                "notes": _row_get(row, "notes", "note", "ملاحظات", "ملاحظة"),
                "image": _row_get(row, "image", "image_url", "photo", "صورة", "رابط الصورة"),
            }
            keyword_parts = [
