...
app = FastAPI(title="PriceBot", version="2.2.0")
...
def ai_enabled() -> bool:
    return config_bool("AI_ENABLED", False) and bool(get_config("AI_API_KEY", "").strip())


def ai_provider() -> str:
    return get_config("AI_PROVIDER", "openrouter").strip().lower() or "openrouter"


def ai_model() -> str:
    return get_config("AI_MODEL", "").strip()


def ai_api_key() -> str:
    raw = get_config("AI_API_KEY", "").strip()
...
def ai_status_text() -> str:
    if ai_enabled():
        model = ai_model() or "default model"
        return f"مفعل: {ai_provider()} / {model}"
    return "غير مفعل"
...
def call_ai_json(user_text: str) -> dict:
    """Call a real external AI API if enabled, returning a parsed JSON dict."""
    if not ai_enabled():
        return {}

    provider = ai_provider()
    key = ai_api_key()
    model = ai_model() or ai_default_model()
    ...
    try:
        if provider == "gemini":
            ...
            # OpenRouter, OpenAI, or any Chat Completions compatible API.
            if provider == "openai":
                url = get_config("AI_BASE_URL", "https://api.openai.com/v1/chat/completions")
            elif provider == "custom":
                url = get_config("AI_BASE_URL", "").strip()
                if not url:
                    print("AI ERROR: AI_BASE_URL is required for custom provider", flush=True)
                    return {}
            else:
                url = get_config("AI_BASE_URL", "https://openrouter.ai/api/v1/chat/completions")
            payload = {
                "model": model,
                "messages": [
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            if parsed:
                print("AI OK:", parsed, flush=True)
            else:
                print("AI PARSE EMPTY", flush=True)
            return parsed
    except urllib.error.HTTPError as exc:
        print("AI HTTP ERROR:", exc.code, exc.read().decode("utf-8", errors="ignore")[:700], flush=True)
    except Exception as exc:
        print("AI EXCEPTION:", str(exc), flush=True)
    return {}
...
@app.get("/")
def home() -> dict:
    return {"status": "PriceBot WhatsApp bot is running", "version": "2.2.0", "business": business_name()}
...
def admin_settings(key: str = "", msg: str = ""):
    ...
    provider = ai_provider()
    current_key = ai_api_key()
    masked_key = (current_key[:8] + "..." + current_key[-6:]) if len(current_key) > 18 else ("محفوظ" if current_key else "غير محفوظ")
    body = f"""
    <h1>إعدادات {e(business_name())}</h1>
    {admin_nav(key)}
    <div class="box">
      <p class="msg">{e(msg)}</p>
      <p class="notice">هنا تضبط اسم الصيدلية والذكاء الاصطناعي الحقيقي. لا يظهر مفتاح API المحفوظ كاملاً.</p>
    </div>
    <div class="box">
      ...
      <form method="post" action="/admin/settings/save?key={urllib.parse.quote(key)}">
        ...
        <h2>الذكاء الاصطناعي الحقيقي API</h2>
        <p class="notice">الحالة الحالية: <b>{e(ai_status_text())}</b> — المفتاح: {e(masked_key)}</p>
        <p><label><input type="checkbox" name="AI_ENABLED" value="yes" {enabled_checked}> تفعيل AI API</label></p>
        <div class="form-grid">
          <div class="field">
            <label>مزود الخدمة</label>
            <select name="AI_PROVIDER">
              <option value="openrouter" {'selected' if provider == 'openrouter' else ''}>OpenRouter</option>
              ...
              <option value="custom" {'selected' if provider == 'custom' else ''}>Custom Chat Completions</option>
            </select>
          </div>
          <div class="field"><label>الموديل</label><input name="AI_MODEL" value="{e(ai_model() or ai_default_model())}" placeholder="مثال: openai/gpt-4o-mini"></div>
          <div class="field"><label>رابط API اختياري</label><input name="AI_BASE_URL" value="{e(get_config('AI_BASE_URL',''))}" placeholder="اتركه فاضي إذا OpenRouter/OpenAI/Gemini"></div>
          <div class="field"><label>مفتاح API جديد</label><input name="AI_API_KEY" type="password" placeholder="الصقه هنا لتغييره فقط"></div>
        </div>
...
async def admin_settings_save(request: Request, key: str = ""):
    ...
    values = {
        ...
        "AI_PROVIDER": str(form.get("AI_PROVIDER", ai_provider())).strip() or "openrouter",
        "AI_MODEL": str(form.get("AI_MODEL", ai_model() or ai_default_model())).strip() or ai_default_model(),
        "AI_BASE_URL": str(form.get("AI_BASE_URL", get_config("AI_BASE_URL", ""))).strip(),
    }
    new_key = str(form.get("AI_API_KEY", "")).strip()
    if new_key:
