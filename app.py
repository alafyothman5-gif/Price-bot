...
async def lifespan(app: FastAPI):
    ...
    print("========================================", flush=True)
    log_event("STARTING", version="stable-v17.4-matching-vision-guard")
...
app = FastAPI(title="PriceBot / WhatsPrice Bot", version="stable-v17.4-matching-vision-guard", lifespan=lifespan)
...
async def health_check():
    ...
    if now - float(health_cache.get("ts") or 0) > 30:
        ...
        health_cache["data"] = {
            ...
            "version": "stable-v17.4-matching-vision-guard",
...
# ---------------------------------------------------------------------------
# FINAL STRICT V17.4 VISION VALIDATION + IMAGE CACHE OVERRIDES
# ---------------------------------------------------------------------------
...
async def handle_image_logic(phone: str, image_id: str, user_state: dict) -> str:  # type: ignore[override]
    ...
    try:
        ...
        if cached and isinstance(cached.get("vision_output"), dict) and cached.get("vision_output"):
            ...
            log_event("VISION_CACHE_HIT", image_hash=image_hash[:12], decision=cached.get("decision"), used_count=cached.get("used_count"))
            return await run_image_matching(phone, ai_data, user_state)
        log_event("VISION_CACHE_MISS", image_hash=image_hash[:12])
