...
def query_product_matches(text: str, limit: int = 8) -> List[dict]:
    ...
    exact = exact_product_in_text(text) or exact_safe_product_match(q)
    if exact:
        return [exact]
...
def build_reply(text: str, from_number: str = "") -> str:
    ...
    if not exact_item:
        exact_item = exact_safe_product_match(extract_product_query(raw_text) or raw_text)
    if exact_item:
        if from_number:
...
def build_unavailable_reply(text: str, from_number: str = "") -> str:
    ...
    exact = exact_product_in_text(text) or exact_safe_product_match(extract_product_query(text) or text)
    if exact:
        if from_number:
