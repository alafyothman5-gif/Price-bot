...
async def analyze_image_with_ai(phone: str, base64_img: str) -> Optional[dict]:
    ...
    prompt_msg = (
        ...
        "Classify product_type as one of: cleanser, sunscreen, serum, shampoo, lotion, moisturizer, cream, drops, spray, tablet, capsule, suppository, injection, unknown. "
...
async def run_image_matching(phone: str, ai_data: dict, user_state: dict) -> str:
    ...
    try:
        direct_image_decision = await asyncio.wait_for(asyncio.to_thread(matcher.resolve_image_query_decision, ai_data), timeout=TEXT_TIMEOUT_SECONDS)
        if direct_image_decision.decision_type.name == "EXACT_MATCH" and direct_image_decision.product:
        ...
        if direct_image_decision.decision_type.name == "ASK_CLARIFICATION":
        ...
        if direct_image_decision.decision_type.name == "COSMETIC_ALTERNATIVES":
        ...
        if direct_image_decision.decision_type.name == "IMAGE_UNCLEAR":
            log_event("FINAL_DECISION", decision="image_unclear_v4", reason=direct_image_decision.reason)
            return matcher.build_unclear_image_reply()
    except Exception as exc:
...
async def lifespan(app: FastAPI):
    ...
    print("========================================")
    log_event("STARTING", version="stable-v16-product-intelligence-v4")
    log_event("PRODUCTS_COUNT", value=database.count_products())
    # PRICEBOT_FAST_STARTUP_READY_V1
    # Do not block FastAPI startup with heavy catalog index warmup. The index is
    # built lazily/background and cached; this prevents "Waiting for application startup" hangs.
    matcher.invalidate_product_cache()
    startup_background_tasks = []
    log_event("PRODUCT_INDEX_READY", value="DEFERRED")
    log_event("MATCHER_V4_INDEX_READY", value="DEFERRED")
    if os.getenv("PRICEBOT_BACKGROUND_WARMUP", "1") == "1":
        startup_background_tasks.append(asyncio.create_task(warmup_matcher_indexes_background()))
    ...
    background_tasks = startup_background_tasks + [asyncio.create_task(spawn_worker(i + 1)) for i in range(WORKERS_COUNT)]
...
app = FastAPI(title="PriceBot / WhatsPrice Bot", version="stable-v16-product-intelligence-v4", lifespan=lifespan)
...
async def health_check():
    ...
    if now - float(health_cache.get("ts") or 0) > 30:
        ...
        health_cache["data"] = {
            ...
            "version": "stable-v16-product-intelligence-v4",
