...
async def run_image_matching(phone: str, ai_data: dict, user_state: dict) -> str:
    ...
    for query in queries:
        ...
        try:
            v2_decision = matcher.resolve_product_query_decision(query)
            if v2_decision.decision_type.name == "EXACT_MATCH" and v2_decision.product:
                ...
                if matcher.is_available(item.get("available", "متوفر")):
                    safe_update_user_state(phone, {"last_product": item})
                else:
                    safe_clear_user_state(phone)
...
async def process_single_message(phone: str, msg_id: str, msg_type: str, msg: dict) -> None:
    ...
    try:
        ...
        log_event("USER_STATE_READ_ERROR", error=repr(exc))
        user_state = {}
    final_reply = ""
    ...
    try:
        ...
        log_event("MARK_MESSAGE_DONE_ERROR", error=repr(exc))
    try:
        database.log_product_inquiry(phone, raw_query, normalized_query, msg_type, final_decision, matched_product, msg_id)
    except Exception as exc:
        log_event("PRODUCT_INQUIRY_LOG_ERROR", error=repr(exc))
    try:
        matcher._log_query_miss_if_needed(phone, raw_query, normalized_query, final_decision)
    except Exception as exc:
        log_event("QUERY_MISS_LOG_ERROR", error=repr(exc))
            ...
            for msg in value.get("messages", []):
                ...
                try:
                    ...
                    try:
                        ...
                        log_event("MARK_FAILED_MESSAGE_ERROR", error=repr(mark_exc))
...
async def lifespan(app: FastAPI):
    ...
    print("========================================")
    log_event("STARTING", version="stable-v14-matcher-v3-strict-engine")
...
app = FastAPI(title="PriceBot / WhatsPrice Bot", version="stable-v14-matcher-v3-strict-engine", lifespan=lifespan)
