...
def merchant_settings(code: str = "", msg: str = ""):
    if not check_merchant(code):
        return merchant_forbidden()
    body = f"""
    <div class="merchant-hero"><h1>إعدادات المحل</h1><p>تعديل البيانات التي تظهر للزبائن، ورمز دخول لوحة التاجر.</p></div>
    {merchant_nav(code)}
    <div class="box">
      <p class="msg">{e(msg)}</p>
      <form method="post" action="/merchant/settings/save?code={urllib.parse.quote(code)}">
        <div class="form-grid">
          <div class="field"><label>رمز دخول لوحة التاجر</label><input name="MERCHANT_CODE" value="{e(merchant_code())}" placeholder="BADR2026"></div>
...
async def merchant_settings_save(request: Request, code: str = ""):
    if not check_merchant(code):
        return merchant_forbidden()
    form = await request.form()
    new_code = str(form.get("MERCHANT_CODE", merchant_code())).strip() or merchant_code()
    values = {
    ...
    for k, v in values.items():
        os.environ[k] = v
    return safe_redirect(f"/merchant/settings?code={urllib.parse.quote(new_code)}&msg={urllib.parse.quote('تم حفظ الإعدادات')}")
...
async def admin_settings_save(request: Request, key: str = ""):
    ...
    values = {
        ...
        "ADMIN_NOTIFY_PHONE": normalize_phone(str(form.get("ADMIN_NOTIFY_PHONE", admin_notify_phone())).strip()),
        "CUSTOMER_WHATSAPP_NUMBER": normalize_phone(str(form.get("CUSTOMER_WHATSAPP_NUMBER", customer_whatsapp_number())).strip()),
        "MERCHANT_CODE": str(form.get("MERCHANT_CODE", merchant_code())).strip() or merchant_code(),
