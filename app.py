...
async def matcher_v3_warmup_loop() -> None:
    """Build/rebuild matcher_v3 index without blocking Uvicorn startup.

    /health must open first.  Customer messages are protected by an
    index-ready guard and receive a short safe reply while the catalog index is
    still warming.
    """
    while True:
        try:
            status = matcher.get_matcher_v3_index_status() if hasattr(matcher, "get_matcher_v3_index_status") else {}
            if not status.get("ready") and not status.get("building"):
                log_event("MATCHER_V3_INDEX_WARMUP_START")
                records = await asyncio.to_thread(matcher.warmup_matcher_v3_cache)
                log_event("MATCHER_V3_INDEX_READY", value="YES", records=records)
            await asyncio.sleep(10)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            log_event("MATCHER_V3_INDEX_WARMUP_ERROR", error=repr(exc))
...
async def analyze_image_with_ai(phone: str, base64_img: str) -> Optional[dict]:
    ...
    prompt_msg = (
        "You are reading a pharmacy/pharmacy-cosmetic product image. Return ONLY valid JSON. "
        "Do not invent stock, price, availability, medical advice, or dose instructions. "
        "If it is a prescription/روشتة, set image_type to prescription. If unclear, set image_type to unclear. "
        "For product packaging, extract the exact front-label brand, product line, and product name. "
        "Classify product_type as one of: cleanser, sunscreen, serum, shampoo, lotion, moisturizer, cream, drops, spray, tablet, syrup, unknown. "
        "Classify target_area as face, body, baby, hair, mouth, or unknown. "
        "For cosmetics, fill skin_concern/usage_purpose such as acne/oily skin, dry skin hydration, sensitive skin, repair/barrier, pigmentation, dandruff/scalp, sunscreen.\n\n"
        "{\n"
...
async def run_image_matching(phone: str, ai_data: dict, user_state: dict) -> str:
    ...
    for query in queries:
        normalized_for_log = matcher.clean_query(query) if hasattr(matcher, "clean_query") else str(query or "")
        log_event("IMAGE_MATCH_TRY", query=query, normalized=normalized_for_log)
        try:
            v2_decision = await asyncio.wait_for(asyncio.to_thread(matcher.resolve_product_query_decision, query), timeout=TEXT_TIMEOUT_SECONDS)
            if v2_decision.decision_type.name == "EXACT_MATCH" and v2_decision.product:
                item = v2_decision.product
                ...
                log_event("FINAL_DECISION", decision="matched_from_image_v2", product=item.get("name", ""))
                return matcher.build_product_reply(item)
            if v2_decision.decision_type.name == "COSMETIC_ALTERNATIVES":
                log_event("FINAL_DECISION", decision="cosmetic_alternatives_from_image_v2")
                return matcher.build_v2_alternatives_reply(phone, v2_decision)
            if v2_decision.decision_type.name == "ASK_CLARIFICATION":
                log_event("FINAL_DECISION", decision="image_ask_clarification_v2", query=query, reason=v2_decision.reason)
                return matcher.build_v2_clarification_reply(phone, v2_decision)
            if v2_decision.decision_type.name == "LOW_CONFIDENCE":
                log_event("FINAL_DECISION", decision="image_low_confidence_v2", query=query, reason=v2_decision.reason)
            ...
            if v2_decision.decision_type.name == "IMAGE_UNCLEAR":
                log_event("FINAL_DECISION", decision="image_unclear_v2", query=query, reason=v2_decision.reason)
                return matcher.build_unclear_image_reply()
            if v2_decision.decision_type.name == "NOT_AVAILABLE":
                saw_clear_product_query = True
                continue
        except Exception as exc:
            if "MATCHER_V3_INDEX_NOT_READY" in str(exc):
                status = matcher.get_matcher_v3_index_status() if hasattr(matcher, "get_matcher_v3_index_status") else {}
                log_event("FINAL_DECISION", decision="image_index_warming", building=status.get("building"), error=status.get("error"))
                return matcher.matcher_v3_warming_reply() if hasattr(matcher, "matcher_v3_warming_reply") else matcher.with_header("النظام يجهز فهرس المنتجات الآن. الرجاء المحاولة بعد لحظات.")
            log_event("MATCHER_V3_IMAGE_ERROR", error=exc)
            return matcher.with_header("لم أتمكن من تحديد المنتج من الصورة بدقة. الرجاء كتابة الاسم الكامل أو إرسال صورة أوضح.")

        # V3 is the final decision engine. Do not fallback to matcher.safe_match.
    ...
    if saw_clear_product_query or visible_text_has_product_signal(ai_data):
        ...
        skin_concern = str(ai_data.get("skin_concern", "") or "").strip()
        usage_purpose = str(ai_data.get("usage_purpose", "") or "").strip()
        unavailable_query = clean_image_query(" ".join(x for x in [brand, product_name, product_type, skin_concern, usage_purpose] if x)) or queries[0]
        log_event("FINAL_DECISION", decision="unavailable_from_image_v3", query=unavailable_query, area=target_area, product_type=product_type)
        await run_db_op("image_clear_state", database.clear_user_state, phone)
        return matcher.with_header("المنتج المطلوب غير موجود أو غير متوفر حالياً في قائمة الصيدلية.")
...
async def lifespan(app: FastAPI):
    ...
    # Keep startup minimal: create DB schema, open HTTP client, spawn workers, then
    # let Uvicorn expose /health.  Heavy product-index work runs in background.
    database.init_db()
    queue = asyncio.Queue(maxsize=QUEUE_MAXSIZE)
    http_client = httpx.AsyncClient(timeout=httpx.Timeout(30.0))
    print("========================================")
    log_event("STARTING", version="stable-v15.2-startup-safe-index-background")
    product_count = await run_db_op("startup_count_products", database.count_products, timeout=1.5, default=0)
    log_event("PRODUCTS_COUNT", value=product_count)
    ...
    background_tasks.append(asyncio.create_task(matcher_v3_warmup_loop()))
    print("========================================")
    try:
        yield
    finally:
...
app = FastAPI(title="PriceBot / WhatsPrice Bot", version="stable-v15.2-startup-safe-index-background", lifespan=lifespan)
...
async def health_check():
    ...
    if now - float(health_cache.get("ts") or 0) > 30:
        previous_count = 0
        try:
            previous_count = int((health_cache.get("data") or {}).get("products_count") or 0)
        except Exception:
            previous_count = 0
        product_count = await run_db_op("health_count_products", database.count_products, timeout=1.5, default=previous_count)
        v3_status = matcher.get_matcher_v3_index_status() if hasattr(matcher, "get_matcher_v3_index_status") else {}
        health_cache["data"] = {
            "ok": True,
            "products_count": product_count,
            "ai_enabled": bool(AI_KEYS_LIST),
            "whatsapp_configured": bool(META_TOKEN and PHONE_ID),
            "version": "stable-v15.2-startup-safe-index-background",
            "queue_size": queue.qsize(),
            "queue_maxsize": QUEUE_MAXSIZE,
            "locks_cached": len(user_locks),
            "matcher_v3_ready": bool(v3_status.get("ready")),
            "matcher_v3_building": bool(v3_status.get("building")),
            "matcher_v3_records": int(v3_status.get("records") or 0),
            "matcher_v3_error": str(v3_status.get("error") or "")[:200],
        }
        health_cache["ts"] = now
