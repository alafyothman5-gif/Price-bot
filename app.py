...
def call_gemini_vision_json(media_bytes: bytes, mime_type: str) -> dict:
    ...
    prompt = (
        "أنت مساعد صيدلية ذكي. مهمتك قراءة أي صورة تصلك وإرجاع JSON فقط بدون أي نص إضافي.\n\n"
        "قواعد التصنيف:\n"
        "1. إذا كانت الصورة تحتوي على علبة دواء/منتج صيدلية واضح أو اسم منتج مكتوب بوضوح → image_type=product_packaging\n"
        "2. إذا كانت الصورة روشتة طبية/ورقة بخط يد/أدوية كثيرة متداخلة → image_type=prescription_or_unclear وrequires_admin_review=true\n"
        "3. إذا كانت الصورة غير واضحة تماماً → image_type=unknown\n\n"
        "للصور من نوع product_packaging:\n"
        "- اقرأ كل النصوص الظاهرة في الصورة بدقة (إنجليزي وعربي)\n"
        "- في product_names: ضع كل الأسماء التي قرأتها من الصورة\n"
        "- في matched_product_names: ضع الأسماء المطابقة أو المشابهة من الكتالوج التالي\n"
        "- الكتالوج: " + json.dumps(catalog_names, ensure_ascii=False) + "\n\n"
        "الشكل المطلوب (JSON فقط، لا تضف أي نص آخر):\n"
        "{\"image_type\":\"product_packaging\",\"product_names\":[\"الاسم كما في الصورة\"],\"matched_product_names\":[\"الاسم من الكتالوج\"],\"requires_admin_review\":false,\"confidence\":0.9}\n\n"
        "مهم: حتى لو لم تجد مطابقة في الكتالوج، ضع الأسماء التي قرأتها في product_names وحافظ على image_type=product_packaging"
    )
...
def product_from_vision(data: dict) -> Optional[dict]:
    ...
    for name in names:
        # 1. Try exact match first via find_product
        item = find_product(name)
        if item:
            return item
        # 2. Try normalize exact match
        n = normalize(name)
        for product in products:
            keys = [product.get("name", "")] + split_aliases(product.get("aliases", ""))
            if any(normalize(k) == n for k in keys):
                return product
        # 3. Try substring match (product name inside vision name or vice versa)
        for product in products:
            pname = normalize(product.get("name", ""))
            if not pname:
                continue
            if pname in n or n in pname:
                if len(min(pname, n, key=len)) >= 4:  # avoid matching tiny tokens
                    return product
        # 4. Try token overlap match (≥60% shared tokens)
        name_tokens = set(search_tokens(name))
        if len(name_tokens) >= 2:
            for product in products:
                full_text = product_full_search_text(product)
                prod_tokens = set(search_tokens(full_text))
                if not prod_tokens:
                    continue
                overlap = len(name_tokens & prod_tokens) / max(len(name_tokens), 1)
                if overlap >= 0.60:
                    return product
...
def process_image_message(msg: dict, from_number: str) -> str:
    ...
    if image_type == "product_packaging" and not requires_review:
        ...
        # Vision read a name but couldn't match — try direct text search on caption + product_names combined
        all_names = list(vision.get("product_names") or []) + list(vision.get("matched_product_names") or [])
        for try_name in all_names:
            if try_name and isinstance(try_name, str):
                fallback = find_product(try_name)
                if fallback:
                    memory_remember_image_product(media_sha, fallback, "gemini_vision_fallback")
                    LAST_PRODUCT[from_number] = fallback
                    return build_product_reply(fallback, caption)
        # If caption has text, also try searching it
        if caption and caption.strip():
            cap_item = find_product(caption)
            if cap_item:
                memory_remember_image_product(media_sha, cap_item, "caption_fallback")
                LAST_PRODUCT[from_number] = cap_item
                return build_product_reply(cap_item, caption)
        # Nothing found — ask admin
        product_names = ", ".join([str(x) for x in all_names if x])
...
def merchant_products(code: str = "", q: str = "", msg: str = ""):
    if not check_merchant(code):
        return merchant_forbidden()
    products = load_products()
    nq = normalize(q)
    ...
    for idx, item in shown:
        cards.append(f"""
        <div class="card">
          <form method="post" action="/merchant/products/save?code={urllib.parse.quote(code)}&idx={idx}" class="card-form">
    ...
    body = f"""
    <div class="merchant-hero"><h1>المنتجات</h1><p>بحث وتعديل السعر والتوفر والملاحظات.</p></div>
    {merchant_nav(code)}
    <p class="msg">{e(msg)}</p>
    <div class="box">
      <form method="get" action="/merchant/products">
        <input type="hidden" name="code" value="{e(code)}">
        <div class="field"><label>بحث</label><input name="q" value="{e(q)}" placeholder="بنادول، بروفين 400، فلاجيل..."></div>
        <div class="actions"><button type="submit">بحث</button></div>
      </form>
    </div>
    <div class="product-grid">{cards_html}</div>
    """
...
async def merchant_products_save(request: Request, code: str = "", idx: int = -1):
    if not check_merchant(code):
        return merchant_forbidden()
    ...
    if 0 <= idx < len(products):
        ...
        save_products(products)
    return safe_redirect(f"/merchant/products?code={urllib.parse.quote(code)}&msg={urllib.parse.quote('تم حفظ التعديل')}")
...
def merchant_upload(code: str = "", msg: str = ""):
    ...
    <div class="box">
      <p class="msg">{e(msg)}</p>
      <p class="notice">الأعمدة المفضلة: name, aliases, active_ingredient, company, form, strength, pack, price, available, notes, image</p>
...
async def merchant_upload_save(request: Request, file: UploadFile = File(...), code: str = ""):
    ...
    if filename.endswith(".xlsx"):
        ...
        rows = [[x.strip() for x in row] for row in csv.reader(io.StringIO(raw)) if row]
    imported = rows_to_products(rows)
    products = [] if replace else load_products()
    products.extend(imported)
    save_products(products)
    return safe_redirect(f"/merchant/upload?code={urllib.parse.quote(code)}&msg={urllib.parse.quote('تم استيراد ' + str(len(imported)) + ' منتج')}")
