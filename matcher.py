...
def resolve_product_query_decision(query: str) -> matcher_v2.MatchDecision:
    """Production-grade resolver entry point used by text and image flows."""
    return matcher_v2.resolve_product_query(query, database.load_products())
...
def handle_text_query_result(phone: str, text: str, user_state: dict) -> QueryResult:
    ...
    try:
        v2_decision = resolve_product_query_decision(text)
        v2_result = _handle_v2_decision(phone, text, v2_decision)
        if v2_result is not None:
            return v2_result
    except Exception as exc:
        print(f"MATCHER_V2_TEXT_WARNING: {exc}")
