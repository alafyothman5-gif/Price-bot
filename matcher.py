...
def _select_pending_variant(phone: str, q_clean: str, user_state: dict) -> Optional["QueryResult"]:
    ...
    exact = _exact_lookup(get_product_index(), q_clean, None)
    if exact and any(str(exact.item.get("id", "")) == str(e.item.get("id", "")) for e in entries):
        item = exact.item
        database.clear_user_state(phone)
        if is_available(item.get("available", "متوفر")):
            database.update_user_state(phone, {"last_product": item})
    ...
    return QueryResult(reply=_ask_variant_reply(phone, entries, user_state.get("pending_variant_kind", "variant"), user_state.get("pending_variant_query", q_clean)), decision="variant_waiting", normalized_query=q_clean)
