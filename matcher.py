...
try:
    ...
    class _FuzzFallback:
        ...
        def partial_ratio(a, b):
            ...
            for i in range(0, max(len(long) - window + 1, 1)):
                best = max(best, difflib.SequenceMatcher(None, short, long[i:i+window]).ratio() * 100)
            return best
...
_V3_INDEX_BUILDING = False
_V3_INDEX_ERROR = ""
_V3_INDEX_BUILT_AT = 0.0
_V3_INDEX_LOCK = Lock()


def invalidate_product_cache() -> None:
    global _PRODUCT_INDEX, _V3_CATALOG_INDEX, _V3_PRODUCTS_COUNT, _V3_INDEX_BUILDING, _V3_INDEX_ERROR, _V3_INDEX_BUILT_AT
    _PRODUCT_INDEX = None
    _V3_CATALOG_INDEX = None
    _V3_PRODUCTS_COUNT = 0
    _V3_INDEX_BUILDING = False
...
def is_matcher_v3_index_ready() -> bool:
    return _V3_CATALOG_INDEX is not None


def get_matcher_v3_index_status() -> Dict[str, object]:
    return {
        "ready": _V3_CATALOG_INDEX is not None,
        "building": bool(_V3_INDEX_BUILDING),
        "records": int(_V3_PRODUCTS_COUNT or 0),
        "error": _V3_INDEX_ERROR,
        "built_at": float(_V3_INDEX_BUILT_AT or 0.0),
    }


def matcher_v3_warming_reply() -> str:
    return with_header("النظام يجهز فهرس المنتجات الآن. الرجاء المحاولة بعد لحظات.")
...
def safe_match(query: str) -> Tuple[str, Optional[dict]]:
    ...
    # Production resolver runs before legacy scoring. It is intentionally conservative:
    # exact/structured/high-confidence only, otherwise ask or unavailable.
    try:
        v2 = resolve_product_query_decision(query)
        if v2.decision_type == matcher_v2.DecisionType.EXACT_MATCH and v2.product:
            return "MATCHED", v2.product
        if v2.decision_type == matcher_v2.DecisionType.ASK_CLARIFICATION:
            return "AMBIGUOUS", None
        if v2.decision_type in {matcher_v2.DecisionType.NOT_AVAILABLE, matcher_v2.DecisionType.COSMETIC_ALTERNATIVES}:
            return "UNAVAILABLE", None
        if v2.decision_type == matcher_v2.DecisionType.LOW_CONFIDENCE:
            return "AMBIGUOUS", None
    except Exception as exc:
        print(f"MATCHER_V2_SAFE_MATCH_WARNING: {exc}")
    ...
    if q_type and not q_brand and len(q_words) <= 3 and not distinct:
    ...
    if q_brand and q_type and not distinct and len(q_words) <= 2:
        if len(candidates) == 1:
    ...
    if q_clean.isdigit():
        ...
        if 1 <= n <= len(options):
            ...
            if is_available(item.get("available", "متوفر")):
                _safe_update_state(phone, {"last_product": item})
...
def get_v3_catalog_index():
    ...
    Live WhatsApp messages must never rebuild the 4,991+ product index.  The
    app starts a background warmup task after Uvicorn opens /health.  If this
    function is called while the index is not ready, it raises a controlled
    error instead of blocking the event loop/customer request.
    """
    if _V3_CATALOG_INDEX is None:
        raise RuntimeError("MATCHER_V3_INDEX_NOT_READY")
    return _V3_CATALOG_INDEX


def warmup_matcher_v3_cache() -> int:
    """Build matcher_v3 index once, with a lock, outside the event loop.

    Safe to call repeatedly from a background task.  It avoids the old
    lru_cache/catalog-key path and builds the production index directly from
    database.load_products().
    """
    global _V3_CATALOG_INDEX, _V3_PRODUCTS_COUNT, _V3_INDEX_BUILDING, _V3_INDEX_ERROR, _V3_INDEX_BUILT_AT
    if _V3_CATALOG_INDEX is not None:
        return len(getattr(_V3_CATALOG_INDEX, "records", []) or [])
    if not _V3_INDEX_LOCK.acquire(blocking=False):
        return 0
    try:
        if _V3_CATALOG_INDEX is not None:
            return len(getattr(_V3_CATALOG_INDEX, "records", []) or [])
        _V3_INDEX_BUILDING = True
        _V3_INDEX_ERROR = ""
        products = database.load_products()
        idx = matcher_v3.build_catalog_index_direct(products)
        _V3_CATALOG_INDEX = idx
        _V3_PRODUCTS_COUNT = len(getattr(idx, "records", []) or [])
        _V3_INDEX_BUILT_AT = time.time()
        return _V3_PRODUCTS_COUNT
    except Exception as exc:
        _V3_INDEX_ERROR = repr(exc)
        raise
    finally:
        _V3_INDEX_BUILDING = False
        _V3_INDEX_LOCK.release()
...
def resolve_product_query_decision(query: str) -> matcher_v2.MatchDecision:
    ...
    message.
    """
    return matcher_v3.resolve_product_query_from_index(query, get_v3_catalog_index())
...
def _v2_option_line(item: dict, index: int) -> str:
    ...
    price_txt = f" - {price} د.ل" if price and "د" not in price else f" - {price}" if price else ""
    return f"{index}) {name}{details_txt}{price_txt}"
...
def build_v2_alternatives_reply(phone: str, decision: matcher_v2.MatchDecision) -> str:
    ...
    if alts:
        ...
        _safe_update_state(phone, {"pending_alternatives": alts[:3]})
    else:
        _safe_clear_state(phone)
...
def build_unavailable_reply(query_clean: str, target_product: Optional[dict], phone: str, explicit_area: str = None) -> str:
    ...
    if alternatives:
        ...
        for index, alt in enumerate(alternatives, 1):
            ...
            prefix = "⭐ أفضل ترشيح: " if index == 1 else f"{index}) "
            body += f"{prefix}{alt.get('name', '')}{price_text}\n"
        body += "\nاكتب رقم البديل للحجز أو الاستفسار."
        _safe_update_state(phone, {"pending_alternatives": alternatives})
    else:
...
def handle_text_query_result(phone: str, text: str, user_state: dict) -> QueryResult:
    ...
    if q_norm.isdigit() and user_state and "pending_alternatives" in user_state:
        ...
        if 0 <= selected_index < len(alternatives):
            ...
            if is_available(selected_item.get("available", "متوفر")):
                _safe_update_state(phone, {"last_product": selected_item})
    ...
    try:
        ...
        if "MATCHER_V3_INDEX_NOT_READY" in str(exc):
            status = get_matcher_v3_index_status()
            print(f"MATCHER_V3_INDEX_NOT_READY: building={status.get('building')} error={status.get('error')}")
            return QueryResult(reply=matcher_v3_warming_reply(), decision="index_warming", normalized_query=q_clean)
