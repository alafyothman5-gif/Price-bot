...
def handle_product_query(query: str, source: str = "text", from_number: str = "", message_id: str = "", deadline: Optional[float] = None) -> str:
    ...
    try:
        ...
        for resolve_query in resolve_queries:
            resolve_timeout = min(4.0 if source == "text" else 8.0, max(0.05, _pb_v5_remaining(deadline) - 0.15))
            if resolve_timeout <= 0.05:
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "timeout_fallback", "STAGE": "resolve_precheck"})
                return _pb_v5_timeout_reply(source)

            candidate_item, status = _pb_v5_call_with_timeout(
                "resolve_product_query",
                lambda q=resolve_query: _pb_v4_resolve_product_query(q, source=source),
                resolve_timeout,
                None,
                message_id,
            )
            if status == "timeout":
                _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "FINAL_DECISION": "timeout_fallback", "STAGE": "resolve_product_query"})
                return _pb_v5_timeout_reply(source)
            if candidate_item:
                item = candidate_item
...
def _pb_v5_process_image_message(msg: dict, from_number: str) -> str:
    ...
    try:
        ...
        if _pb_v5_expired(deadline, 0.1):
            _pb_v5_log("FINAL_DECISION", {"MESSAGE_ID": message_id, "SOURCE": "image", "FINAL_DECISION": "timeout_fallback", "STAGE": "before_handle_product_query"})
            return _pb_v5_timeout_reply("image")
