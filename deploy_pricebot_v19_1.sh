...
if [[ -x "$BACKUP_SCRIPT" ]]; then
  ...
  tar --exclude='./venv' --exclude='./.venv' --exclude='./__pycache__' -czf "/root/pricebot_backups/pricebot_pre_v19_1_$(date +%Y%m%d_%H%M%S).tar.gz" .
fi
...
print('PRICEBOT_V19_1_SAFE_MIGRATION_OK')
PY
