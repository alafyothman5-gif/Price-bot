...
def _rules() -> List[Tuple[str, str]]:
    ...
    try:
        from database import load_dynamic_synonyms_clean
        combined.update(load_dynamic_synonyms_clean())
    except Exception:
        pass
...
def _availability_unknown(value: Any) -> bool:
    norm = normalize_product_text(value or "")
    if not norm:
        return True
    unknown = {"unknown", "غير معروف", "غير محدد", "uncertain", "n/a", "na", "-"}
    return norm in unknown


def _availability_ok(value: Any) -> bool:
    # Empty/unknown catalog availability must not be treated as available.
    norm = normalize_product_text(value or "")
    if not norm:
        return False
    bad = {"غير متوفر", "غير موجود", "نافذ", "نفذ", "ناقص", "out of stock", "unavailable", "no", "0", "unknown", "غير معروف", "غير محدد", "n/a", "na"}
...
def to_product_record(item: Dict[str, Any]) -> ProductRecord:
    ...
    return ProductRecord(
        ...
        availability=_field(item, "available", "availability"),
        raw=item,
    )
...
def _availability_decision(rec: ProductRecord, slots: QuerySlots, ci: CatalogIndex) -> MatchDecision:
    ...
    if _availability_unknown(rec.availability):
        # Identity is resolved, but stock is unknown. The response builder must
        # not allow reservation or say available.
        return MatchDecision(DecisionType.EXACT_MATCH, confidence=0.95, product=rec.raw, product_record=rec, reason="v3_exact_unknown_availability", query_slots=slots)
    if rec.is_cosmetic:
...
def _strict_missing_cosmetic_alternatives(slots: QuerySlots, records: List[ProductRecord], limit: int = 3) -> List[ProductRecord]:
    ...
    # is strong and exact. This version is stricter about skin/use: if the user
    # asks for oily/acne/dry/sensitive, matching products are preferred and wrong
    # known skin/use products are excluded.
    target_type = slots.cosmetic_type or slots.form
    if target_type not in {"cleanser", "sunscreen", "shampoo", "serum"}:
        return []
    preferred: List[ProductRecord] = []
    fallback: List[ProductRecord] = []
    for rec in records:
        ...
        # If user specified skin/use and catalog record has a different known target, exclude it.
        ...
        exact_skin = bool(slots.skin_type and rec.skin_type == slots.skin_type)
        exact_use = bool(slots.use_case and rec.use_case == slots.use_case)
        same_brand = bool(slots.brand and rec.brand == slots.brand)
        if (slots.skin_type or slots.use_case) and (exact_skin or exact_use):
            preferred.append(rec)
        elif not (slots.skin_type or slots.use_case):
            preferred.append(rec)
        else:
            # Unknown skin/use is allowed only as a fallback when no direct target exists.
            fallback.append(rec)
    ...
    scored = sorted(
        ...
        key=lambda r: (
            r.brand == slots.brand,
            bool(slots.skin_type and r.skin_type == slots.skin_type),
            bool(slots.use_case and r.use_case == slots.use_case),
            fuzz.token_set_ratio(slots.cleaned_text, r.identity),
        ),
        reverse=True,
    )
    return scored[:limit]
...
# PRICEBOT_FAST_INDEX_BUILD_V2
# The original build_catalog_index created a huge tuple cache key from the whole
# SQLite catalog. On a real 4,991-product catalog this can make app startup look
# frozen. The app already keeps one process-level index in matcher.py, so a huge
# lru_cache key here is unnecessary.
_ORIGINAL_NORMALIZE_PRODUCT_TEXT = normalize_product_text
_ORIGINAL_CONTAINS_PHRASE = _contains_phrase

@lru_cache(maxsize=100000)
def _pricebot_normalize_cached(value: str) -> str:
    return _ORIGINAL_NORMALIZE_PRODUCT_TEXT(value)


def normalize_product_text(text: Any) -> str:  # type: ignore[override]
    return _pricebot_normalize_cached(str(text or ""))


@lru_cache(maxsize=200000)
def _pricebot_contains_phrase_cached(text: str, phrase: str) -> bool:
    text_norm = normalize_product_text(text)
    phrase_norm = normalize_product_text(phrase)
    return bool(text_norm and phrase_norm and f" {phrase_norm} " in f" {text_norm} ")


def _contains_phrase(text: str, phrase: str) -> bool:  # type: ignore[override]
    return _pricebot_contains_phrase_cached(str(text or ""), str(phrase or ""))


def _pricebot_record_identity(self: ProductRecord) -> str:
    cached = self.__dict__.get("_pricebot_cached_identity")
    ...
    value = normalize_product_text(" ".join(x for x in parts if x))
    self.__dict__["_pricebot_cached_identity"] = value
    return value


def _pricebot_record_tokens(self: ProductRecord) -> Set[str]:
    cached = self.__dict__.get("_pricebot_cached_tokens")
    if cached is not None:
        return cached
    value = set(tokenize(self.identity))
    self.__dict__["_pricebot_cached_tokens"] = value
    return value


ProductRecord.identity = property(_pricebot_record_identity)  # type: ignore[assignment]
ProductRecord.tokens = property(_pricebot_record_tokens)      # type: ignore[assignment]


def build_catalog_index(catalog: Sequence[Dict[str, Any]]) -> CatalogIndex:  # type: ignore[override]
    records = [to_product_record(item) for item in catalog if _field(item, "name", "original_name")]
    ci = CatalogIndex(records=records)
    for rec in records:
        _add_index(ci.exact_name_index, rec.normalized_name, rec)
        for alias in rec.aliases | rec.image_keywords:
...
def refresh_synonym_rules() -> None:  # type: ignore[override]
    global _RULES_CACHE
    _RULES_CACHE = None
    try:
        ...
        pass
    try:
        _pricebot_normalize_cached.cache_clear()
        _pricebot_contains_phrase_cached.cache_clear()
    except Exception:
        pass
