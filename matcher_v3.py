...
def refresh_synonym_rules() -> None:
    ...
    _RULES_CACHE = None
    try:
        _normalize_product_text_cached.cache_clear()
    except Exception:
        pass
...
def normalize_product_text(text: Any) -> str:
    # This function is called thousands of times while indexing a 4,991+ product
    # catalog.  Keep the public wrapper tolerant of any value, but cache the
    # string-normalization path aggressively.
    return _normalize_product_text_cached(str(text or ""))


@lru_cache(maxsize=200000)
def _normalize_product_text_cached(text: str) -> str:
...
def build_catalog_index_direct(catalog: Sequence[Dict[str, Any]]) -> CatalogIndex:
    """Build a CatalogIndex directly from product dictionaries.

    Production must not create a giant tuple cache key for the real 4,991+
    product catalog during startup or live WhatsApp requests.  The process-level
    cache in matcher.py owns the lifetime of this index; this function only
    performs the deterministic in-memory build.
    """
    records = [to_product_record(dict(item)) for item in catalog if _field(dict(item), "name", "original_name")]
...
def get_catalog_index(catalog_key: Tuple[Tuple[Tuple[str, str], ...], ...]) -> CatalogIndex:
    # Test/demo path only. Production uses build_catalog_index_direct through
    # matcher.get_v3_catalog_index() to avoid costly cache-key construction.
    return build_catalog_index_direct([dict(pairs) for pairs in catalog_key])
