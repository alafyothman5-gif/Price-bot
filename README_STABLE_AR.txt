...
stable-v13 matcher_v2 safe resolver additions:
- Added rapidfuzz fuzzy matching with safe fallback, improving tolerance for typos like Cearave and بنادووول.
- Added extended Arabic/phonetic normalization: Arabic/Persian letters, Arabic digits, and repeated-letter simplification.
- Added query_logs table and admin /admin/misses page to collect failed/unavailable/ambiguous searches for continuous improvement.
- Added dynamic synonyms table and admin /admin/synonyms + /admin/addsynonym to add new words from the dashboard without code redeploy.
- Dynamic synonyms reload matcher rules and invalidate product cache immediately.
- Improved cosmetic alternatives scoring with context similarity, same-type filtering, and first item marked as best recommendation.
- Smart unavailable replies now distinguish between product not listed and known product currently unavailable.
- Product cache invalidation is called after product edit/upload/delete and after synonym changes.
- This release keeps v11 monster search, variant resolver, image alternatives, queue/rate/cleanup, admin analytics, and DB namespace fixes.
...
stable-v13.1 matcher_v2 review fix:
- Image flow now respects ASK_CLARIFICATION / LOW_CONFIDENCE / IMAGE_UNCLEAR from matcher_v2 and does not fall back to legacy safe_match for those decisions.
- matcher_v2 loads dynamic synonyms from database.load_dynamic_synonyms() and can refresh them without restart.
- Added acceptance tests for image clarification no-fallback, dynamic synonyms, and Cerave baume type safety.

stable-v14 matcher_v3 strict engine:
- تمت إضافة matcher_v3.py كمحرك قرار نهائي آمن بدل الاعتماد على safe_match القديم.
- مسار النص والصورة يستخدم resolve_product_query من matcher_v3 ولا يعود إلى fuzzy/safe_match القديم.
- القرارات النهائية محصورة في EXACT_MATCH / ASK_CLARIFICATION / NOT_AVAILABLE / COSMETIC_ALTERNATIVES / LOW_CONFIDENCE / IMAGE_UNCLEAR.
- أضيفت acceptance_tests_v3.py لاختبارات Rilastil/CeraVe/Flagyl/Panadol والصور غير الواضحة والبدائل الآمنة.
- أضيف catalog_quality_report.py لتوليد catalog_quality_report.csv من قاعدة المنتجات الحالية.
- الأدوية لا تحصل على بدائل علاجية عشوائية. الكوزمتك لا تحصل على بدائل إلا من نفس النوع وبشروط صارمة.
