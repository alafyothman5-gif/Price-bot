...
def _brand_or_type_only_guard(slots: QuerySlots, ci: CatalogIndex) -> Optional[MatchDecision]:
    ...
    if slots.brand and not specific and not slots.form and not slots.strength_values and not slots.size_values:
        ...
        return matcher_v3._ask("يرجى تحديد المنتج المطلوب من هذه الشركة:", options, slots, "product", "v4_brand_only") if options else MatchDecision(DecisionType.LOW_CONFIDENCE, reason="v4_brand_only_no_catalog", query_slots=slots)
    if (slots.cosmetic_type or slots.form) and not slots.brand and not specific and not slots.strength_values:
        ...
        return matcher_v3._ask("يرجى تحديد الشركة أو المنتج المطلوب:", options, slots, "product", "v4_type_only") if options else MatchDecision(DecisionType.LOW_CONFIDENCE, reason="v4_type_only_no_catalog", query_slots=slots)
    return None
...
def resolve_image_extraction_from_index(ai_data: Dict[str, Any], ci: CatalogIndex) -> MatchDecision:
    ...
    if image_type in {"unclear", "other", "unknown"} or clarity == "bad" or confidence < 0.70:
